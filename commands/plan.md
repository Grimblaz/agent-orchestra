...
1. Require an issue number (the plan is posted as a durable comment on that issue). If missing, ask for one.
2. Check the issue's comments/timeline for the `<!-- design-phase-complete-{ID} -->` marker (design completion lives on a comment, not in the issue body), then route (#957 D4, default posture):
   ...
   - **No marker, but the issue carries an affirmed open-for-work framing record** — either recognised form, per `skills/open-for-work/SKILL.md` § Resuming an issue already opened for work (the registered marker family, or an unedited comment whose first line matches the interim practiced form's exact literal; read comments via `gh api repos/{owner}/{repo}/issues/{ID}/comments --paginate`, which carries `updated_at` and does not stop at the first page, rather than `gh issue view`, which carries neither) — → **run that section's state decision, do not assume beat 2 is next.** It is order-sensitive; run that section's state decision, which owns the full outcome set — several of its outcomes forbid authoring here, including: an artifact predating the earliest lawful record is **not lawful under source (b)** (report, do not resume under it), and an issue already carrying a `plan-variant: brief` plan comment newer than its latest record is **`routed`** — the plan comment family is upsert-in-place, so re-authoring would replace a reviewed brief rather than append. Only `affirmed-not-routed` and `re-affirmed-not-re-routed` **resume the conversation at beat 2** (#957 Amendment 10): classify the still-open list against the affirmed what-statement. A **routine** outcome proceeds to a **brief** under authority source (b) — do not ask to run `/design` first; the record plus the routine verdict are the lawful authority. A **novel** outcome continues into `/design` instead. The record alone, with beat 2 unrun, does not authorize a brief. **Say who posted the record.** That `gh api` response already carries `user.login` and `author_association`. Name the author of **every record this state decision relied on** — at minimum the one you are acting under **and** the earliest lawful record when they differ, since the earliest is what the ordering check reads — **in every outcome, including the ones that do not resume**. § Resuming's **Who posted it** paragraph carries the output shapes; the *reason* it is disclosure rather than a check, and what a person should do when the author is not who they expected, are one further hop on at § Writing the affirmation record under **Trust model**. Disclosure only: **nothing this pre-flight computes is conditioned on the author** — recognition, lawfulness, and the computed state are unchanged, and a record from any author is recognised exactly as before. **But do not silently route past a surprise**: if anything about the disclosed author looks off to you, present it and stop for the person before beat 2, or before continuing under an existing routing artifact; otherwise disclose and proceed as normal, because a record you did not write is the ordinary case. Stopping to show a person is a **handoff, not a computed route**, and the judgment that follows is theirs, not yours.
   - **Neither marker nor framing record** → for standalone work the expected route is the **open-for-work entrance**, not the phase pipeline. Offer the user a choice: (a) run `/open {issue}` (recommended for standalone work — one conversation to a lawful brief or a continuation into design), (b) run `/design` first, or (c) plan from whatever framing already exists — the phase pipeline remains lawful on explicit request. *(This pre-flight still does not recognize a chunk sub-issue of a designed parent — #924 owns that wiring. On a chunk sub-issue, the interim rule in `Documents/Design/chunked-delivery.md` § Deferred follow-up applies unchanged: decline these offers, cite that doctrine, and author the brief under authority source (a).)*
