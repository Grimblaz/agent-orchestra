...
2. Check the issue's comments/timeline for the `<!-- design-phase-complete-{ID} -->` marker (design completion lives on a comment, not in the issue body), then route (#957 D4, default posture):
   ...
   - **No marker, but the issue carries an affirmed open-for-work framing record** — either form in `Documents/Design/open-for-work.md` § The affirmation record, including the interim practiced form — → author the plan as a **brief** under authority source (b). Do not ask to run `/design` first; the framing record is the lawful authority.
   - **No marker and no framing record, and the issue is a chunk sub-issue of a designed parent** → the interim behavior in `Documents/Design/chunked-delivery.md` § Deferred follow-up holds until #924 lands: decline the `/design` offer, cite that doctrine, and author a brief under authority source (a).
   - **No marker and no framing record, standalone** → the expected route is the **open-for-work entrance**, not the phase pipeline. The `/open {issue}` command is pending (#957 chunk 3), so the entrance runs manually today: offer, via `AskUserQuestion`, to (a) run the open-for-work conversation in this session per `Documents/Design/open-for-work.md` § Running the flow today (recommended), (b) run `/design` first, or (c) plan from whatever framing already exists — the phase pipeline remains lawful on explicit request.
