...
Its reported failure count is **not** a count of tests — it adds one per failing *file* on top of the failing assertions, so a run reporting 13 may be 7 failing tests across 6 files. Build any baseline measurement in a detached worktree (`git worktree add <path> <commit> --detach`): a `git stash` cannot prove a failure is pre-existing, and a `git archive` extract has no repository metadata and fails a large number of tests for that reason alone. [`Documents/Design/test-suite-baseline-948.md`](../../Documents/Design/test-suite-baseline-948.md) records the last full-suite baseline, the runner's counting and false-green traps in detail, and the disposition of every test that was red at it.
