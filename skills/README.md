...
| `agent-memory-compaction` | Compaction policy for an agent memory store's recall index, plus the sweep it runs on: the rules a compaction may not break, how an entry leaves and where that exit is recorded, the slate procedure and its dispositions, the size budget the index is held to, and a re-runnable four-axis check | ✅ Included |
...
| `open-for-work` | Operating methodology for opening a filed standalone issue for work: the trivial floor, the worth-it doors, beat 1 alignment with its affirmation gate and durable record, beat 2 routing, and the two outputs. Loaded by `/open`. DO NOT USE FOR: chunk sub-issues of a designed parent (author the brief directly per plan-authoring § Brief plan variant). For issue close-out mechanics, with or without a PR: the record's two firing moments and its advisory force are in plan-authoring § The close-out obligation on an affirmation-record issue; its shape and lawfulness lookup are in post-pr-review § 9, which states both moments too; and this skill's own § Close-out is the stated reader for the close-time backstop. When a later review sustains findings an existing record does not carry, the amendment's trigger and procedure are in review-judgment § Close-Out Record Amendment, its single home on every lane that runs a judge. | ✅ Included |
...
| `review-judgment` | Single-shot review judgment and scoring methodology, plus the parent-owned post-judge steps: § Close-Out Record Amendment (the close-out record's amendment trigger — its single home, named by every lane that runs a judge) and § Post-Judge Disposition Gate | ✅ Included |
