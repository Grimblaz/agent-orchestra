...
`filtered_count` is consumed, not merely validated: it supplies the brief-review dismiss-rate's numerator and denominator in the review-cost report, where convergence-filtered findings are this surface's dismissed population.
...
**A self-certification contradiction is refused**: a plan comment declaring `plan-variant: brief` whose ledger sibling carries a judge-rulings head renders could-not-verify naming the contradiction. Scoped to the ledger sibling, so a code-review judge head elsewhere on the issue does not trip it. Note the honest bound: because the check requires the declaration, it would **not** have caught #939 or #941 — neither declares `plan-variant` at all. It closes the path for every brief from #956 onward.
