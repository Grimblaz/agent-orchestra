...
**Known gap — the phase-containment ledger has no brief emission path yet** (raised by the #947 review, routed to #936 as amendment **DA6**). `### Phase-containment emission` below is judge-gated end to end: `persist-phase-ledger.ps1 -Mode plan` requires judge-rulings content, one block per *judge-sustained* finding, and the emission check's honest fallback keys on the `**Plan Stress-Test**` literal this section mandates. A brief's review produces no judge ruling, so a brief renders `plan-stress-test: COULD NOT VERIFY` until DA6 defines its emission surface. This is stated rather than worked around: suppressing the `**Plan Stress-Test**` literal to dodge the fallback would produce a silent zero, which is exactly the false-clean 811-D1 exists to prevent. Until DA6 lands, record the review outcome in the `Plan Stress-Test` prose as normal and expect the ledger row to read as a gap.
