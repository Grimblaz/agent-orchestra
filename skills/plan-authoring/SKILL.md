...
**Two firing moments, stated together.** A rule carrying only the first cannot reach an issue that closes without a pull request, and that population is not hypothetical — one of the six is exactly that case.
...
2. **Close-time backstop, on a run that will not.** An issue closed by hand with no pull request gets the record written **before the close**. `skills/open-for-work/SKILL.md` § Close-out and `skills/post-pr-review/SKILL.md` § 9 are this same obligation met at that moment.
...
- **Provisional until the PR merges.** A record written at moment 1 describes a run that has not landed. Until the merge it is provisional, not a discharged obligation.
- **A second PR amends the existing record rather than posting a new one.** Two records on one issue read as two close-outs, and a later reader cannot tell which is current.
- **Amend when late findings are sustained, and the firing surface is named**: `skills/code-review-intake/SKILL.md` § Response Loop Completion. When that terminal sequence sustains a finding after the record was written, the record is amended there. This rule carries its own trigger because an unamended record is *present* and therefore reads as discharged — harder to notice than an absence, so the presence-beats-ordering argument that justifies the rest of this section does not transfer to it.

**What the run writes** — the record's three items and its exact shape stay at `skills/post-pr-review/SKILL.md` § 9. Close-Out Record (Issues Opened For Work). This section owns *when* the obligation is read and *that* it is advisory; that step owns the record's content.
