...
- **Transcript slug derivation preserves case and maps every non-alphanumeric character to a dash — but only after path-shape- and platform-specific normalization and a degenerate-input guard run first.** The substitution phase (`cwd.replace(/[^a-zA-Z0-9]/g, '-')`, capped at 200 characters with a base-36 hash suffix appended beyond that) never collapses, trims, or lowercases — so `C:\Users\Micah\Code 2\copilot-orchestra` yields `C--Users-Micah-Code-2-copilot-orchestra` and a `.claude\worktrees\` checkout yields `...-Orchestra--claude-worktrees-...` (the dotted segment contributes its own dash on top of the separator's). But `Get-CostTranscriptSlug` is not that substitution alone: it first rejects a path with no ASCII alphanumeric character (returns `''`), then normalizes MSYS/separator/trailing-separator path shapes to match what Claude Code's own `process.cwd()` would report — normalization that *does* trim, on the reasoning that a normalized string is what should feed the (still trim-free) substitution. And the 200-character cap bounds the substituted prefix specifically; the appended `-<base36-hash>` suffix sits beyond it, not inside it. Issue #908 corrected the substitution rule itself from an earlier per-segment join that lowercased the drive and only substituted spaces, which derived a directory name that never existed for worktree checkouts. If the Claude projects directory uses a different slug format in practice, adjust the derivation in `cost-walker.ps1` rather than trying to rename directories.
