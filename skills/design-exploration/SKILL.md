...
description: "Reusable technical design exploration methodology. Use when researching design options, grounding UI changes in the current experience, converging trade-offs into one recommended design direction, proposing a mechanism because a process step did not happen, grounding a chosen option against its rejected alternatives, deciding what an agent judges and what deterministic code checks, designing a record format that claims a property, or designing a rule that must tell a declaration from text quoting it. DO NOT USE FOR: GitHub issue update ownership, adversarial design challenge orchestration, or approval-policy enforcement (keep those in Solution-Designer.agent.md)"
---
...
- [references/design-exhibits.md](references/design-exhibits.md): the incident detail behind § Design Lenses — the doctrine records, grounding passes, format defects and failed heuristics each lens was extracted from, including the six-point agentic-tooling creed in full
...
Five ways a design converges on something defensible and wrong. Incident detail sits in [references/design-exhibits.md](references/design-exhibits.md), cited per lens.
