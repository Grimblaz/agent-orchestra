...
2. Build a finding ledger where each item maps to its GitHub comment/review ID and, so that per-reviewer disposition evidence is compilable from judge rulings, records a per-finding `reviewer_source` field (canonical field name per 768-D3 — never a flat `source:` key). Normalize the value deterministically: `lowercase(author.login)` with a trailing `[bot]` suffix stripped, sourced from the author's `.login` field only (never display name), and written as a quoted scalar. The reserved values `local` (pipeline-native findings only) and `unresolved` (lookup-failure sentinel) can never be emitted by this normalization; a real login that collides with a reserved word is recorded as `ext-{login}`. When one review comment enumerates multiple distinct findings, each becomes its own ledger item sharing that comment/review ID — per-finding key identity for that case is defined by the discriminator in `skills/review-judgment/SKILL.md`'s § Stable Finding Key section.
