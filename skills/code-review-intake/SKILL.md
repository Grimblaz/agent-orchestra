...
2. Build a finding ledger where each item maps to its GitHub comment/review ID and, so that per-reviewer disposition evidence is compilable from judge rulings, records a per-finding `reviewer_source` field (canonical field name per 768-D3 — never a flat `source:` key). This is the canonical statement of the `reviewer_source` normalization rule — other sites (`skills/review-judgment/SKILL.md`, `Documents/Design/phase-containment-ledger.md`) cite this section rather than restating the mechanism. Normalize the value deterministically: `lowercase(author.login)` with a trailing `[bot]` suffix stripped, sourced from the author's `.login` field only (never display name), and written as a quoted scalar. The reserved values `local` (pipeline-native findings only) and `unresolved` (lookup-failure sentinel) can never be emitted by this normalization, and neither can a value that already looks escaped: the escape fires when the normalized login IS one of the reserved values OR already STARTS WITH the `ext-` prefix, and in either case it is escaped by prepending `ext-` — so `local` → `ext-local`, a real login `ext-local` → `ext-ext-local`, and `ext-unresolved` → `ext-ext-unresolved`. This single conditional prepend (not an unbounded loop) is sufficient to keep the escaped and unescaped namespaces collision-free, because it recognizes any already-escaped-looking value, not just an exact reserved word. When one review comment enumerates multiple distinct findings, each becomes its own ledger item sharing that comment/review ID — per-finding key identity for that case is defined by the discriminator in `skills/review-judgment/SKILL.md`'s § Stable Finding Key section.
...
- [references/response-loop-completion.md](references/response-loop-completion.md): ordered terminal-step sequence (disposition gate, R4 dispatch, post-fix prosecution, CE Gate, persist-changes, close-out record amendment, Response Summary) that completes the `/review-github` response loop
...
The ordered terminal-step sequence that makes a bare `/review-github` complete the full response loop (disposition gate, Batch Specialist Dispatch R4, post-fix targeted prosecution, CE Gate, persist-changes, close-out record amendment, and Response Summary) lives in [references/response-loop-completion.md](references/response-loop-completion.md). Load that reference once all judgment states reach terminal; accepted fixes are applied, committed, and pushed to the existing PR branch without requiring an additional user instruction.

**This section is the named firing surface for the close-out record's amendment rule** (`skills/plan-authoring/SKILL.md` § The close-out obligation on an affirmation-record issue). On an issue carrying an open-for-work affirmation record whose close-out record was already written, findings this loop sustains after that point are folded into the **existing** record by amending it in place — never by posting a second one. It is advisory: a skipped amendment is reported in the Response Summary and never halts the loop. The step's placement in the sequence and its scoping are in the reference above; the rule exists because an unamended record looks discharged, which is harder to notice than an absent one. When a sustained finding's `reviewer_source` (normalized above) resolves to a real external identity, the judge's phase-containment emission may owe a `post-review-observer` block instead of the standard `code-review` block — see `skills/review-judgment/SKILL.md` § Phase-containment emission § Observer emission variant for the novel-gating rule and exact-equality dispatch; this section does not restate that mechanism.
