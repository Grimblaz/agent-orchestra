...
PR #988 replaced an allowlist with glob-minus-`.github/scripts/Tests/ci-quarantine.json` selection. Skipping a suite now means adding an entry carrying a `class` and a `reason`. The classes at ship were: `unclassified` — never measured on Linux, a backlog rather than a decision, 189 entries at ship, all linked to #993; `linux-red` — measured failing on CI, an issue number required; and `never-ci` — structurally unable to run in CI (live `gh`, network, interactive terminal), permanent and legitimate. **`unclassified` no longer exists.** #1035 measured the whole corpus on Linux and #1036 spent that measurement, retiring the class outright rather than leaving it to shrink by good intentions: `Get-CISuiteSelection` now refuses an entry carrying it, and the legal set is `linux-red`, `never-ci`, and `no-signal` — the last for a suite that executes and yields no verdict at all, which also requires an issue because the condition producing it is expected to end. Derive the live counts from `Get-CISuiteSelection` rather than reading a figure here. `ci-suite-registration.Tests.ps1` fails the job if any suite on disk is neither run nor listed, if an entry names a deleted file, if a reason is empty, or if the registry is missing or malformed. It is itself selected by the glob, so it cannot be dropped without dropping its own reporter.
