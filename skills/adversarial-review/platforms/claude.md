...
| `design-challenge` | `prosecution` | `n/a` | 3 | `Review mode selector: "Use design review perspectives"` | Required for every Code-Critic prosecution pass; use the parallel-batch policy for the three-pass design challenge | Defense omitted; judge omitted | Skip; prosecution-only and non-atomic | No local review-state persistence; Solution-Designer incorporates or dismisses findings and updates the issue body |
