...
5. **Close-out record amendment outcome, one per judge pass this run reached** — amended (naming what it added), no-new-findings, not-applicable (naming which precondition failed: no issue resolved, no affirmation record, or no existing record), or skipped with its reason, carrying the `⚠️ close-out record amendment skipped — {reason}` literal when skipped. State how many passes fired. Required because that step is **advisory** and never halts the loop, so this summary is its only accountability channel on this lane; without the item a skipped amendment disappears into a summary that still satisfies every other requirement here. The step itself — trigger, moment, executor, and procedure — lives at `skills/review-judgment/SKILL.md` § Close-Out Record Amendment, its single home on every lane; this item reports its outcome and restates none of it.
