...
**Post-fix scope constraint** - Post-fix prosecution and judgment evaluate fix-introduced regressions and direct side effects; coverage (including mutation testing) extends to every branch the fix commit modified. Findings unrelated to the fix diff changes (pre-existing style issues, optimization opportunities in untouched code, general code quality concerns in surrounding area) must be classified as DEFERRED-SIGNIFICANT regardless of severity, except that an in-diff pre-existing finding mapping to an explicit acceptance criterion item routes through the AC Cross-Check Gate before deferral. The out-of-diff AC exception is preserved: if a finding outside the diff maps to an explicit acceptance criterion item, the AC Cross-Check Gate applies.
