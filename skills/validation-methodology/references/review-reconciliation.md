...
> **Survival**: `SMC-05` covers the pre-PR `/memories/session/review-state-{ID}.md` lookup as `within-conversation` state. `SMC-06` covers post-PR resume as `durable` review comment or PR-body metrics first, with session memory last.
...
**Disposition filter (review-dispositions gate)**: Before dispatching any sustained finding to a specialist, resolve each finding's disposition. The **in-session disposition set** assembled by the Post-Judge Disposition Gate in the current run (`skills/review-judgment/SKILL.md § Post-Judge Disposition Gate`) is the PRIMARY source — read it directly, do not wait for or re-fetch the posted comment. Consult the posted `<!-- review-dispositions-{PR} -->` marker only as a FALLBACK, for a context that lacks the in-session set (e.g. a resumed or reconstructed session with no in-memory gate output). Skip findings where `disposition: escalate` — those were explicitly excluded from the current PR's fix scope by the engineer during the post-judge disposition gate. Only `disposition: incorporate` entries proceed to fix dispatch. **Fail closed**: if neither the in-session set nor the posted marker is available for a finding, treat its disposition as unresolved and do not dispatch it — an `escalate`-classified (or unresolvable) finding must never reach fix dispatch by default; only entries with a confirmed `disposition: incorporate` proceed.
...
**Post-fix scope constraint** - Post-fix prosecution and judgment evaluate fix-introduced regressions and direct side effects; coverage (including mutation testing) extends to every branch the fix commit modified. Two parallel dispositions follow from that scope:

1. **Out-of-diff findings** - Findings unrelated to the fix diff changes (pre-existing style issues, optimization opportunities in untouched code, general code quality concerns in surrounding area) must be classified as DEFERRED-SIGNIFICANT regardless of severity, except that an out-of-diff finding mapping to an explicit acceptance criterion item routes through the AC Cross-Check Gate instead of deferral.
2. **In-diff pre-existing findings** - Within the fix diff itself, the widened branch coverage can also surface pre-existing (non-fix-introduced) findings. These in-diff pre-existing findings are likewise classified as DEFERRED-SIGNIFICANT unless they map to an explicit acceptance criterion item, in which case they route through the AC Cross-Check Gate before deferral.
...
5. **Close-out record amendment outcome, one per judge pass this run reached** — exactly one of `amended` (naming what it added), `no-new-findings`, `not-applicable` (an issue was resolved but carries no affirmation record or no existing record — name which), `no-issue-resolved`, or `skipped` with its reason; the last two carry the `⚠️ close-out record amendment skipped — {reason}` literal. State how many passes fired. Required because that step is **advisory** and never halts the loop, so this summary is its only accountability channel on this lane; without the item a skipped amendment disappears into a summary that still satisfies every other requirement here. The step itself — trigger, moment, executor, and procedure — lives at `skills/review-judgment/SKILL.md` § Close-Out Record Amendment, its single home on every lane; this item reports its outcome and restates none of it.
