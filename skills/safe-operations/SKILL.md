...
5. Read **every** comment on those surfaces — not the newest marker per phase — and collect every `followup-`-prefixed entry from every engagement-record marker found. Use the uncapped read (`Get-FollowupPriorMarkerBodies`); a `gh ... --json comments` fetch caps at 100 comments and can shadow the record on a busy thread.
   ...
   - **The sixth failure path is your own call, and the exit-code probe cannot see it.** The five paths above are all transport; a caller that mis-invokes the helper gets the same bare empty set while `gh` exits `0`, so the probe reports the surface readable and every filing reads *unsupported*. Verified the hard way on the first execution of this procedure (2026-08-08): `Get-FollowupPriorMarkerBodies` takes **`-Number`**, not `-IssueNumber`, and returns **objects carrying `.Body`**, not bare strings — getting either wrong yields zero records against a live, record-bearing surface. Neither mistake throws if the call sits in a `try`/`catch`, so do not wrap it in one. Discriminate the same way any absence claim is discriminated here: run one **positive control** — point the reader at a surface known to carry a ruling record and require it to come back non-empty before you trust a single *unsupported* in the batch. A run whose control returns nothing has measured its own reader, not the corpus.
...
> **And the detected class is narrower still: it is a session that omits the record write *while stamping a ruling-asserting value*.** Domain membership is decided at step 2 by the provenance stamp, and that stamp is self-attested — `-FilingProvenance` is a ValidateSet, so the filer picks its own value and nothing ties any value to what actually happened. A session that omits the record write and stamps `direct-request` is therefore not caught by a weaker verdict; it is **out of domain**, and the procedure stops before looking. Note what that means for #1011, the exhibit this contract was written around: the same session, changing one enum value, reads clean. So `direct-request` carries the identical structure #1012 was filed to close — a self-attested claim about a maintainer action that leaves no artifact — one value to the side of the mechanism that now checks it. This is a **known, unclosed** boundary, not a solved one; it is stated here so no reader mistakes an empty domain for a checked corpus. Live as of 2026-08-08: of 49 provenance-stamped filings, 15 are out of domain, and **every one of the 5 filed since this contract landed** is `direct-request` — the procedure has never returned `located` for any filing, ever.
