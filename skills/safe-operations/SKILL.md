...
- **Approve** — file the issue as proposed, with `Add-FollowUpIssue -OriginatingPr {surface} -FilingProvenance 'gate-approved'`. The `-OriginatingPr` anchor is **not optional here**: see "Anchor requirement" below.
- **Modify** — the maintainer edits the title, scope, or severity; the edited title re-runs dedup per "Modify-re-dedup" above, and (absent a dedup hit) files with `-OriginatingPr {surface} -FilingProvenance 'gate-modified'` — the same mandatory anchor.
...
| `gate_ruling_decided_at` | the decision timestamp: the same UTC-to-the-second string hashed into the key |
...
4. Otherwise the stamp asserts a ruling. Derive the search surfaces from the filing: the `originating_pr` field of the `<!-- code-conductor-filed-followup -->` sentinel block, and the `Parent: #N` line. Resolve each against the repo the filing lives in unless the value is repo-qualified. For `queue-consumed`, the `<!-- proposed-followups-{PR|ISSUE} -->` comment on those same surfaces is also in scope — but see step 5's queue note: the queue payload alone never satisfies `located`.
...
5. Read **every** comment on those surfaces — not the newest marker per phase — and collect every `followup-`-prefixed entry from every engagement-record marker found. Use the uncapped read (`Get-FollowupPriorMarkerBodies`); a `gh ... --json comments` fetch caps at 100 comments and can shadow the record on a busy thread.
   - **Check that the read succeeded.** That helper returns an empty set on five distinct failure paths — unresolvable repo, a `gh api` exception, a non-zero `gh` exit (an expired token or a rate limit lands here), empty output, and an unparseable response — and one of them emits no warning at all. An empty set that came from a *failed* read means the outcome is **could-not-verify**; stop there and say so. Reporting a transport failure as "unsupported" converts an infrastructure problem into an accusation, which is exactly what the failed-write branch above refuses to do on the write side. **But note the asymmetry the anchor introduces**: the anchor is filer-supplied, so a filing naming a surface that does not exist, or one in a repo the reader cannot read, also lands here — and that is a compliance defect in the filing, not a neutral outcome. A `could-not-verify` whose cause is an anchor that does not resolve reads as *not-reconcilable*; only a failure independent of the filing's own values (a rate limit, an expired token, a network fault) is genuinely inconclusive.
6. The outcome is **located** when some ruling record in that set has a `gate_ruling_items` entry whose `title` matches this issue's title with an `outcome` matching the stamp (`approved` for `gate-approved`; `modified` for `gate-modified`; either for `queue-consumed`), and whose `gate_ruling_decided_at` is at or before the issue's own `createdAt`. Otherwise the outcome is **unsupported**.
   ...
   - **Queue note.** A `<!-- proposed-followups-… -->` payload carries titles, dispositions, and board positions but no counts and no per-item outcome, so nothing in it can satisfy `located`. It is in scope only as a pointer to where the consuming session's ruling record should be; the record itself is what the verdict rests on.
