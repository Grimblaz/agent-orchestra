...
| --- | --- | --- | --- |
| Halt-report body rendering | `New-GoalRunHaltCommentBody` (`.github/scripts/lib/goal-run-halt-core.ps1`) | `live-interaction` | `halt-claim` entry when the halt originated from the executor's own claim, corroborated by an `experience-observation` entry recording Experience-Owner's live render check of the rendered body |
...
| Second-invocation refuse/adopt/complete decision report | `agents/Goal-Run.agent.md` Invocation Contract step 2, backed by `Resolve-GoalRunInvocationAction` (`refuse-resume-existing` \| `adopt-and-resume` \| `resolve-and-report-complete`) + `Test-GoalRunInflightAppearsDead` (`.github/scripts/lib/goal-run-stage-core.ps1`) | `live-interaction` | `experience-observation` entry (Experience-Owner triggers each of the three decisions against a live, a dead-with-no-terminal-outcome, and a dead-with-terminal-outcome inflight marker, and observes the reported action) |
...
| Surface | Real function / file | evidence_type | Run-log provenance |
| --- | --- | --- | --- |
| Halt-report comment posting | `Invoke-GoalRunHaltEmit` (`.github/scripts/lib/goal-run-halt-core.ps1`) | `code-audit` | `halt-claim` entry when the halt originated from the executor's own claim; the exerciser reads the emit implementation and cross-references the durable posted comment against that entry rather than re-posting a halt comment live |
