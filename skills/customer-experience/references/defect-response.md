...
2. Route the deferred defect through the `§2e Filing Approval Gate` (`skills/safe-operations/SKILL.md` § 2e) rather than filing directly. Once the maintainer approves (or modifies) the proposal, **write the batch's ruling record first** (§ 2e "Ruling record") — that write is a precondition of filing, not a follow-up, and if it fails the filing does not proceed. Then call `Add-FollowUpIssue -AcCrossCheck -OriginatingPr {PR} -FilingProvenance 'gate-approved'` (or `'gate-modified'` if the maintainer edited the proposal) so the follow-up issue body carries AC-provenance for the deferral decision, the required filing-provenance stamp, and the surface anchor § 2e's reconciliation reader searches from.
