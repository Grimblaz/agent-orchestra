...
3. **BDD Phase 2 Runner Dispatch** (conditional - skip entirely when Phase 2 is not active; Phase 2 requires `## BDD Framework` **line-start heading** (column 0) AND `bdd: {framework}` line with recognized framework in the winning file (see `skills/bdd-scenarios/SKILL.md` § BDD Detection Mechanism — `AGENTS.md › CLAUDE.md › copilot-instructions.md`)):
   ...
   6. **Evidence merge**: after step 4 (EO delegation) returns, merge EO evidence for all delegated scenarios. Reachable conflict: runner-fail `[auto]` scenario where EO yields a pass -> `source: runner+eo`, `result: conflict` (`evidence_type: automated-runner+live-interaction` — see the compound-literal rule in skills/bdd-scenarios/SKILL.md). (Note: runner-pass + EO-fail is unreachable - runner-passed `[auto]` scenarios are excluded from EO delegation by sub-step v above.)
...
9. CE Gate result markers (emitted by the judge in conjunction with Code-Conductor's read of the verdict):
   ...
   The four passing markers above (strong/partial/weak/passed-after-fix) are rendered with an evidence-mix suffix, e.g. `(evidence: live 4/6, code-audit 2/6)` — counts are of scenarios that entered the evidence record; when the denominator is 0 or all scenarios are INCONCLUSIVE, omit the suffix entirely rather than rendering a 0/0 fraction.
