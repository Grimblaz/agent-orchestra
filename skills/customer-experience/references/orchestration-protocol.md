...
3. **BDD Phase 2 Runner Dispatch** (conditional - skip entirely when Phase 2 is not active; Phase 2 requires `## BDD Framework` **line-start heading** (column 0) AND `bdd: {framework}` line with recognized framework in consumer repo's `copilot-instructions.md`):
