...
Code-Conductor/D9 owns durable handoff persistence. Continue implementation stays same-session and session-memory-only, with no redundant GitHub issue comments. Stop, Pause, or resume later paths persist or append durable GitHub issue comments for `<!-- plan-issue-{ID} -->` and `<!-- design-issue-{ID} -->` when those artifacts are needed. When the session-memory plan carries `slice_comment_id` (authored under the split plan/frame-slices shape), a durable re-emit must preserve that sibling structure — do not inline frame-slice blocks back into the plan comment.
