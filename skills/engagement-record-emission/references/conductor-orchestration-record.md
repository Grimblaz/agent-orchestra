...
- **Two-entry burst manifest (893-D4, script-enforced ordering)**: When scope-classification resolves, Code-Conductor authors two payload files under `.tmp/issue-{ID}/` with the Write tool — the comment body carrying the `<!-- engagement-record-orchestration-{ID} -->` marker and Markdown mirror, then the `<!-- credit-input-orchestration-{ID} -->` body carrying:
  ...
  It builds a burst manifest naming both entries in that order (family `engagement-record` first, family `credit-input` second) and invokes `skills/session-memory-contract/scripts/persist-marker.ps1 -BurstManifest <manifest-path>` exactly once — this is the ONLY documented write path for the burst; never a hand-composed `gh issue comment` call. **What that rule buys is a single audited writer — not protection from `updated_at` advancement.** Both families in this burst are `post-new`, so the write POSTs fresh comments and advances nothing; the hazard arrives when a write replaces a **whole body** already carrying someone else's marker. See `skills/session-memory-contract/references/handoff-markers.md` § What the write-path rule buys for the write-shape split. The script's own halt-on-first-failure burst execution is what enforces the ordering: on engagement-record write failure, the burst halts immediately and the credit-input entry is reported not-attempted in the returned artifact manifest.
