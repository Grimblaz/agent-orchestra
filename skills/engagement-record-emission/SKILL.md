...
**Also a writer (#974): the open-for-work conversation.** `skills/open-for-work/SKILL.md` records its checkpoint decisions — the worth-it doors, the affirmation gate, and brief approval on the routine arm — into `<!-- engagement-record-experience-{ID} -->` at `schema_version: 2`, because that conversation *is* the experience-replacement for standalone work (#957 D5). It is not an upstream agent, so it is named here explicitly rather than inferred, on the same footing as the `orchestration`-phase carve-out below. **Cumulative-write obligation**: that conversation reaches three checkpoints at different moments, and this family is `post-new` with latest-createdAt-wins on read — so a later marker carrying only the later decision orphans the earlier one. Write one marker at conversation exit carrying every decision, or make each later write carry the full accumulated set.
...
**Write path (issue #893)**: `skills/session-memory-contract/scripts/persist-marker.ps1` (family `engagement-record`, `post-new`, with a payload-only validator adapter that parses the candidate via `Read-EngagementRecords -InMemoryMarkers` before any network write) is the ONLY documented write path for the `experience`, `design`, `plan`, and `orchestration` phases — never a hand-composed `gh issue comment` call. **What that rule buys is a single audited writer — not protection from `updated_at` advancement.** This family is `post-new`, so the write POSTs a fresh comment and advances nothing; the hazard arrives when a write replaces a **whole body** already carrying someone else's marker. See `skills/session-memory-contract/references/handoff-markers.md` § What the write-path rule buys for the write-shape split. **`review`-phase markers (`engagement-record-review-{PR}`) are a known v1 registry gap and stay hand-authored**: the family registry declares `TargetSurface: issue` uniformly, but the `review` phase is PR-keyed, so a `review`-phase write through the script is refused by the surface preflight until that gap is closed.
...
| Comment-burst halt-on-failure | If engagement-record emission fails after the completion marker is posted, credit-input would still be posted. | Script-enforced (issue #893): the phase-exit burst is one `persist-marker.ps1 -BurstManifest` invocation whose halt-on-first-failure execution stops before credit-input when engagement-record write fails — the completion marker (already landed) stands, the phase is still marked complete, and the script's `⚠️` warning line must be relayed to the terminal. |
