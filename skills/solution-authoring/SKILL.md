...
Firing-position rule: for an atomic multi-stage adapter declaring `integrity-contract.atomic: true` and more than one `pipeline-stages` entry, the classification gate fires only on judge-sustained findings after the terminal stage. The gate does not fire on prosecution output or defense dialogue inside the atomic window. CLAUDE.md Engagement-gate non-overridability D3 still applies: once that post-judge input exists, the gate fires unconditionally; only the firing position changes for atomic multi-stage adapters. For prosecution-only adapters (`design-challenge`, `lite`), the gate fires on merged-ledger findings as today.
...
Re-audit timing: under an atomic multi-stage adapter, re-audit triggers apply to the judge-sustained-findings set, which is the input to the post-judge classification gate. For prosecution-only adapters, re-audit applies to merged-ledger findings as today. Re-audit cannot retroactively invalidate a judge ruling; it can only revise the maintainer disposition classification/decision.
