...
When a parent emits multiple `Agent` calls in one parallel tool-use block (for example `commands/plan.md` and `commands/orchestra-review.md` prosecution × 3), per-dispatch recapture is satisfied by a **single live recapture immediately before the parallel block**, with one handshake block constructed per dispatch from those captured values. Every dispatch in the batch carries **one shared UTC ISO-8601 `handshake_issued_at` timestamp**, captured with the other values (a single `-IssuedAt` for the whole batch), so all six handshake fields — the four tree fields (`parent_head`, `parent_branch`, `parent_cwd`, `parent_dirty_fingerprint`), `workspace_mode`, and `handshake_issued_at` — are field-identical across the batch. Sharing the timestamp keeps same-model panel prompts byte-identical through the entire handshake block so they can share a cached prompt prefix (#975); it is sound because the four tree fields are already recaptured once for the batch and the batch is dispatched from one instant. (The timestamp remains a distinct per-dispatch value across *sequential* stages, which recapture — see below.)
