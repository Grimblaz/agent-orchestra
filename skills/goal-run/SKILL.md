...
- **State file** — `New-GoalRunActiveState` writes `goal-run-active.json` at the worktree root: ceilings, baseline, arm, executor session id, the launch-pinned `contract_hash`, `launched_at`, a `heartbeat_at` timestamp (seeded equal to `launched_at`), and `teardown_deferred` (default `false`). `Get-GoalRunActiveState` reads it back and returns `$null` on a missing file rather than throwing. `Update-GoalRunActiveStateHeartbeat` refreshes `heartbeat_at` — wired live (M5 fix) via `Invoke-GoalRunChainStageBoundaryHousekeeping` ([goal-run-budget-core.ps1](../../.github/scripts/lib/goal-run-budget-core.ps1)) at every chain-stage boundary and at the loop-release point; before that fix the function existed and was tested but had zero live callers.
- **Run log** — `goal-run-log.jsonl` at the worktree root (alongside `goal-run-active.json`), one JSON object per line, each independently validated against [`schemas/goal-run-log.schema.json`](schemas/goal-run-log.schema.json) before it is appended. `Add-GoalRunLogEntry` ([goal-run-log-core.ps1](../../.github/scripts/lib/goal-run-log-core.ps1), M9 fix) is the schema-validating writer; `Test-GoalRunLogHasCheckpoint` is the reader `Resolve-GoalRunResumeStage`'s `-RunLogHasCheckpoint` parameter is actually fed from (Invocation Contract step 3). A `checkpoint` entry is written at every chain-stage boundary via the same `Invoke-GoalRunChainStageBoundaryHousekeeping` call named above.
...
- **Launch-pinned contract-hash check** (step 5, wired live by the M1 fix) — `Test-GoalRunContractHashPinned` ([goal-run-prompt-core.ps1](../../.github/scripts/lib/goal-run-prompt-core.ps1)) compares the *live* issue-body contract hash against the hash pinned at launch time, not the contract's own self-reported `contract_hash` field — this catches a post-approval edit that kept its own field internally consistent. A mismatch is the highest-precedence halt producer, `invariant-conflict`. Three live call sites actually run this check: the vendor `/goal` loop predicate ([goal-run-predicate.ps1](../../.github/scripts/goal-run-predicate.ps1) / [goal-run-predicate-core.ps1](../../.github/scripts/lib/goal-run-predicate-core.ps1), on every loop iteration, via `Resolve-GoalRunLoopPredicate`), chain re-validation (`Invoke-GoalRunChainRevalidate`'s `-LaunchPinnedHash` parameter, [goal-run-chain-core.ps1](../../.github/scripts/lib/goal-run-chain-core.ps1)), and resume-time verification (the `agents/Goal-Run.agent.md` Invocation Contract step 1). Before the M1 fix, the function existed and was tested but had zero live callers.
