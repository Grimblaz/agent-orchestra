...
- **Current branch**: flags the current branch when its upstream was merged/deleted. A current `claude/*` no-upstream branch is a candidate on branch shape alone — prefix and upstream state — since issue #922 removed the ancestry pre-filter that used to gate it. That filter rendered a merged verdict of its own, and because a squash merge writes a new commit, a squash-merged branch is never an ancestor and was discarded before any evidence was gathered. Candidacy only becomes eligibility through the shared evidence-gated eligibility primitive (`Test-WorktreeBranchRemovalEligible`): git-only tree-equivalence or an OID-checked merged-PR-by-head match, or — for a branch with no unique commits — an OID-checked merged PR or a closed parent issue. When the signal is unverifiable, the branch is reported as needing manual review (with the specific reason) rather than silently retained or falsely offered for cleanup. The shared `Test-WorktreeIsPrimary` guard excludes the primary checkout from this candidate check before eligibility is ever evaluated. When eligible, the current branch — whether it has an issue ID (`feature/issue-N-…`) or not — is rolled into the same composite `pwsh ... post-merge-cleanup.ps1 ... -FeatureBranch '<branch>'` invocation as the other categories, so confirming cleanup triggers exactly one permission prompt. The script switches to the default branch, pulls, and deletes the now-stale current branch on your behalf. (The `Documents/Design/session-hooks.md` note about running current-worktree cleanup from another checkout still applies if you choose to bypass the script and run the underlying git commands manually — but the recommended path is the composite invocation.)
...
- **Sibling worktrees**: flags sibling worktrees on `claude/*` no-upstream branches or upstream-deleted `feature/issue-*` branches, then routes each candidate through the same shared evidence-gated eligibility primitive (`Test-WorktreeBranchRemovalEligible`) as the current-branch check above — git-only tree-equivalence or an OID-checked merged-PR-by-head match for a branch with unique commits, or an OID-checked merged PR or closed parent issue for a branch with none. A candidate whose signal is unverifiable is reported as needing manual review (`Sibling worktree branch <branch> at <path> needs manual review: <reason>`) instead of being silently dropped or falsely offered for cleanup. The shared `Test-WorktreeIsPrimary` guard excludes the primary checkout from sibling candidacy on every code path, including the case where the agent runs from a non-primary linked worktree and the primary checkout would otherwise appear to be a sibling. Eligible candidates' cleanup is rolled into the single composite `pwsh ... post-merge-cleanup.ps1 -SiblingWorktrees @(...) -OrphanBranches @(...)` invocation appearing inside the fenced PowerShell block — not as individual `git` commands. Cleanup time re-verifies eligibility independently rather than trusting the detector's snapshot, then reports the verified post-removal state honestly: `removed`, `removed-partial-root-held` (contents gone but an empty directory remains — another process held the root), `removed-partial-content-remains` (unregistered but files remain — inspect manually), `stale-registration` (the removal attempt did NOT succeed in deregistering the worktree — the directory is gone but `git worktree list` still shows it registered; this is a failed-to-deregister outcome and is never counted toward the "Deleted N sibling worktree(s)" summary, unlike the other outcomes above), `failed`, or `verification-indeterminate` — never the old false "skipping" message for a worktree a removal attempt had already partially destroyed. A locked worktree whose directory still exists is left for manual review and never force-removed on the lock flag alone (`skipped locked worktree ... - remove the lock first`). A locked worktree whose directory is confirmed absent via `Test-Path` (issue #522, corrected) is double-force-removed (`git worktree remove --force --force`) regardless of the porcelain `prunable` marker — real git never sets `prunable` on a locked worktree, so the corrected clause keys on `Test-Path`-verified absence alone, never on the (for a locked worktree, unreachable) `prunable` self-report — and the actual post-attempt state is then verified through the same honest outcome probe, never assumed.
- **Orphan branches**: flags unattached merged `claude/*` no-upstream branches and unattached upstream-deleted `feature/issue-*` branches. Their cleanup is rolled into the single composite `pwsh ... post-merge-cleanup.ps1 -SiblingWorktrees @(...) -OrphanBranches @(...)` invocation appearing inside the fenced PowerShell block — not as individual `git` commands. Squash-merged branches are classified as merged from content evidence — strict tree-equivalence (`git diff --quiet` exit 0) or a merge-tree no-op. Since issue #922 `git cherry` establishes neither verdict: it compares patch identities, so a revert leaves it reporting merged and a squash leaves it reporting unmerged for a fully shipped branch. Both no-upstream and upstream-deleted candidates are routed at detection time through the same shared evidence-gated eligibility primitive (`Test-WorktreeBranchRemovalEligible`) used by the sibling and current-branch checks above; a candidate whose signal is unverifiable is reported as needing manual review (with the specific reason) instead of being silently dropped. `feature/issue-N-*` orphan branches — the session-convention naming this repo uses — carry an additional `; eligible for auto-resolve at cleanup time` suffix: the detection-time verdict is a report, not the final word, because the destructive `git branch -D` at cleanup time always re-evaluates live rather than trusting the detector's snapshot. For a zero-unique-commit `feature/issue-N-*` or `claude/*-N-hex` branch with a derivable issue id (the two session-convention naming shapes the shared primitive can derive an issue id from), cleanup time reuses the same shared primitive as detection (OID-checked-PR-first, then closed-issue evidence). For every other `feature/issue-N-*` case — one or more unique commits, or the unique-commit count itself could not be determined (a `git rev-list` failure) — cleanup time instead uses the richer tree-equivalence-plus-absorption chain (`Test-BranchMergedIntoDefault` + `Test-OrphanBranchAutoResolveEligible`): the parent issue must be CLOSED, a merged PR with matching `headRefOid` must exist, and every residual commit must be absorbed by main (ancestor-on-main / patch-equivalent / spike-only confined to `.tmp/issue-N/` / tree-at-HEAD-matches-main). Honest skip messages at cleanup time: `auto-resolve declined` (signals say not shipped; branch retained), `could not verify auto-resolve signals` (gh unavailable — fail-open, branch retained), and `branch not reachable from default (merged-state re-check returned false)` (a race between detection and the pre-delete re-check). Non-`feature/issue-N-*` orphans (e.g., `claude/*`) never carry the auto-resolve suffix or use the absorption chain. A zero-unique-commit non-`feature/issue-N-*` orphan with a derivable issue id (`claude/*-N-hex`) clears only through the shared primitive's own zero-commit evidence (OID-checked-PR-first, then closed-issue evidence), same as described above. A non-`feature/issue-N-*` orphan with one or more unique commits instead falls through to `Test-BranchMergedIntoDefault` directly: git-only tree-equivalence, or — when that is inconclusive — the same OID-checked merged-PR-by-head lookup the primitive itself uses (never a name-only `gh pr list` match, which would misreport a branch that merely shares its name with an unrelated merged PR as shipped).
- **Fail-open behavior**: fetch, worktree-list, for-each-ref, and ref-lookup failures suppress only the unverifiable candidate and do not fail the startup session.
- **Deliberate silence for live work (issue #922)**: a candidate the eligibility gate concludes is *definitively unmerged* — content evidence shows it carries changes absent from the remote default, and no merged pull request matches its current tip — renders **nothing at all**: no finding line, no command argument, no contribution to the ten-line display cap. This is the one class the check deliberately says nothing about, because it is live work rather than a leftover, and a manual-review line for every in-flight worktree at every session start is noise that trains you to ignore the block. Every other not-eligible candidate is reported with its cause. Silence is reached only after the candidate has been *evaluated* to that conclusion, never by declining to look at it.
...
```json
{
  "permissions": {
    "allow": [
      ...
      "Bash(pwsh*persist-marker.ps1*)"
    ]
  }
}
```

The first entry covers the composite cleanup-script invocation that the session-startup hook emits. The second and third entries cover edge cases where manual cleanup commands are needed outside the script (for example, when the current-worktree narrative guidance is followed manually from another checkout). The fourth entry (issue #893) covers `skills/session-memory-contract/scripts/persist-marker.ps1`, the marker-write primitive that supersedes hand-composed `gh issue comment` / `gh pr comment` calls for every migrated marker family (see `skills/session-memory-contract/references/handoff-markers.md`).

This allowlist entry is safe specifically because of `persist-marker-core.ps1`'s scratch-root bounding (issue #893 slices s6/s11, root check added by the PR #917 review's F1 fix): `Resolve-MarkerScratchBoundedPath` requires every `-BodyFile`/burst-manifest `bodyFile` to resolve, via real `Resolve-Path` canonicalization (traversal-aware — never a raw string-prefix test on the unresolved input) plus a symlink/junction reparse-point check on the scratch root itself AND a per-segment walk of every descendant down to the leaf (`Resolve-Path` alone does not dereference reparse points, so both the root and every descendant must be checked explicitly), inside the consumer repo's `.tmp/` scratch root before the script will read it. Known residual gap: hard links are not reparse points and are not detected by this walk. Without that bound, a broad `Bash(pwsh*persist-marker.ps1*)` allowlist entry would let any `-BodyFile` argument read an arbitrary file on disk and post its contents to a public GitHub comment — an arbitrary-file-read-to-public-comment primitive. The bounding is what keeps the wide `pwsh*persist-marker.ps1*` glob pattern (rather than a narrower, harder-to-match pattern) safe to allowlist: a payload path outside `.tmp/` is refused pre-write, before any network call.
...
| `git cherry` labels squash-merged branch commits as unique (patch-id false positive) | Squash-merged branches are reported as "unmerged" and left behind instead of being cleaned up | Merge-detection uses `git diff --quiet` tree-equivalence as primary check, then a merge-tree no-op; `git cherry` was removed as a conclusive signal in both directions by issue #922 |
