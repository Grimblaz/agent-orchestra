...
- **`.tmp/` scratch clearing**: per-issue `.tmp/{N}-*` and `.tmp/issue-{N}*` scratch files are cleared at cleanup time (not detected) by passing `-TmpRoot .tmp -IssueNumber N` to `post-merge-cleanup.ps1`; the `Remove-IssueTmpScratch` function handles removal and emits a summary line.
