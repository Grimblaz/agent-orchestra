...
This allowlist entry is safe specifically because of `persist-marker-core.ps1`'s scratch-root bounding (issue #893 slice s6): `Resolve-MarkerScratchBoundedPath` requires every `-BodyFile`/burst-manifest `bodyFile` to resolve, via real `Resolve-Path` canonicalization (symlink- and traversal-aware — never a raw string-prefix test on the unresolved input), inside the consumer repo's `.tmp/` scratch root before the script will read it. Without that bound, a broad `Bash(pwsh*persist-marker.ps1*)` allowlist entry would let any `-BodyFile` argument read an arbitrary file on disk and post its contents to a public GitHub comment — an arbitrary-file-read-to-public-comment primitive. The bounding is what keeps the wide `pwsh*persist-marker.ps1*` glob pattern (rather than a narrower, harder-to-match pattern) safe to allowlist: a payload path outside `.tmp/` is refused pre-write, before any network call.
