...
**What surfaces to the user**: Silent by default. Step 7c speaks up on every successful snapshot; this step is quieter — it speaks up only when it actually upgrades a PR's numbers, showing a single line: `upgraded #{pr} to end-of-session`. Every other outcome (no eligible candidate this session, a candidate that was checked but didn't qualify for the upgrade, or any failure) stays silent, because those are all expected fail-open outcomes rather than something the user needs to act on — surfacing them would just be startup noise for a background housekeeping task.
