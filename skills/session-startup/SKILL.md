...
This allowlist entry is safe specifically because of `persist-marker-core.ps1`'s scratch-root bounding (issue #893 slices s6/s11): `Resolve-MarkerScratchBoundedPath` requires every `-BodyFile`/burst-manifest `bodyFile` to resolve, via real `Resolve-Path` canonicalization (traversal-aware — never a raw string-prefix test on the unresolved input) plus a per-segment symlink/junction reparse-point walk from the scratch root down to the leaf (s11 M1 fix — `Resolve-Path` alone does not dereference reparse points), inside the consumer repo's `.tmp/` scratch root before the script will read it. Known residual gap: hard links are not reparse points and are not detected by this walk. Without that bound, a broad `Bash(pwsh*persist-marker.ps1*)` allowlist entry would let any `-BodyFile` argument read an arbitrary file on disk and post its contents to a public GitHub comment — an arbitrary-file-read-to-public-comment primitive. The bounding is what keeps the wide `pwsh*persist-marker.ps1*` glob pattern (rather than a narrower, harder-to-match pattern) safe to allowlist: a payload path outside `.tmp/` is refused pre-write, before any network call.
