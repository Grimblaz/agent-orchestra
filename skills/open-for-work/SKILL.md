...
> **No lawful record** on #{ID}: {n} candidate record(s) found, all void as ordering witnesses because each was edited after creation — posted by {login} at `{created_at}`, edited `{updated_at}`. The issue has no source-(b) authority; not proceeding.

The earlier-record variant belongs on **every** shape that names a record, not only the first: an earlier record is exactly what lets a routing artifact pass step 2, so `routed` is one of the outcomes it matters most in. And note the last shape names the authors of *voided* records: an edited record is skipped for every ordering purpose, which makes editing the cheapest way to keep a record out of a disclosure that only ever named lawful ones. Name them anyway.
