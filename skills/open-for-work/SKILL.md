...
**Who posted it — disclose it, never condition on it.** The same response carries `user.login` and `author_association` for every comment; no extra fetch is needed. Carry the author of every record you collect alongside its timestamps, and **say it in what you present**. This is not a field a reader could *merely* have looked up had they thought to — it is part of the summary they are handed, in every outcome below.

**Name every record the state decision relied on — at minimum — and never just the one you are resuming under.** (The collection duty above is the wider one: carry every record's author. This names the floor for what must reach the output.) This is the part that is easy to get wrong, and getting it wrong is what makes the disclosure decorative. The two checks below read *different* records: step 2's ordering check reads the **earliest lawful** record, and step 3's supersession check reads the **latest lawful** one. When those are the same record, one line covers it. **When they differ, name both** — otherwise the record that supplied lawfulness can be invisible while the output names another. That is not hypothetical: an earlier record is exactly what lets a routing artifact pass step 2, and step 2 is described below as the only moment a later reader can catch a back-fitted authority.
...
> **Nothing to resume** on #{ID} (state `complete`); the issue carries a lawful record posted by {login} at `{created_at}`.

Those cover the four states below plus the two stop outcomes. If you land somewhere with no shape here, the obligation still stands — write the author line in whatever wording the outcome calls for. The shapes are worked examples, not an allowlist.

Render the login plain, without a leading `@` — a rendered `@mention` carried into an issue comment notifies that account, which is a gratuitous signal when the comment you are writing is one declining to resume under their record.
