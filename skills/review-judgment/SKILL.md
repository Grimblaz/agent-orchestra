...
**Two shortcuts are forbidden here, both documented mis-write traps** (same reference): `gh issue comment --edit-last` targets the author's most recent comment on the issue, which is whatever the run posted last rather than the record; and `Find-OrUpsertComment`'s `-Body` **replaces** rather than appends, and selects its target by a `-like` match that finding text quoting a marker can capture. A mis-targeted write here edits some other comment on the issue, which is why the id comes from the same read that recognised the record and never from a guess.
