...
- **The last two rostered lessons land as lenses** (#1051, chunk 3 of umbrella #1045). Parent amendment A5 was ruled by the maintainer: A4.5's scope extends to admit both contradictions chunk 2 routed up. Both lessons promote carrying a `conflict` verdict together with its resolution — the state amendment A4.5 made expressible, exercised here for the first time. The promotion roster is now 46 of 46 terminal: 19 + 25 + 2 promoted, none pending.
- **The standing check gained a containment clause.** Pinning the receiving-skill set in the caller stopped the manifest shrinking that population, but nothing stopped the caller's set from failing to *cover* the manifest — and an uncovered skill is never visited, so its unmanifested-file scan and its `## Composite References` arm were both skipped while the audit reported zero drift. Found by running it: the audit returned no drift both with and without the receiving entry this chunk had to hand-add. Closed, and pinned by fixtures asserting the discriminating pair, the orphan-scan silence, and containment-not-equality.
- **An AC6 store reconciliation record** (`Documents/Planning/lesson-promotion-store-reconciliation.md`), covering all 46 rows across both tracks amendment A2.3 names, with the store-side snapshot fingerprinted and the record pinned by `in_file_pins`. No CI check can perform the manifest↔store join — a runner gets a clone and the store is not in it — so the record is the only durable evidence, and its reader is the 2026-09-08 sweep.

### Fixed

- **`skills/plugin-release-hygiene/SKILL.md` § 5 no longer derives the next version from the branch.** It reads the default branch with a fetch immediately before the read, names the shell the command actually runs in (MSYS argument conversion breaks it under Git Bash), refuses the branch-local fallback outright, and names `Get-PRHDefaultBranch`'s last two rungs as the stop condition rather than an answer — rung 6 returns your own working branch and rung 7 assumes the literal `main`. The re-check now fires before landing rather than only after a merge, which is how #1050's second failure got through.
- **The same section states its own bound.** Reading the default branch shows what has landed, not what an open PR has claimed, so two branches cut from one base still collide; `release-gate.ps1` is a landing backstop, not a prevention, and is itself suppressed while a PR is conflicted. The claimed-version recipe now reads each open head's manifest rather than listing branch names that carry no version. Structural fix tracked at #864.
- **The grounding-evidence detector is described as it behaves, at every surface that described it.** The first correction removed one absolute and introduced two more: it called the two span exclusions "the whole of what this detector decides" (omitting the adjacency and line-anchor rules) and named "an unfenced, column-0 paste" as the uncaught shape (narrower than reality, and pinning on column position the exact heuristic this tranche's own lens records as failing). Now stated as behaviour: only *fenced* quoting and inline code spans are excluded, an *indented* code block reads `canonical` at any indentation, and blockquote/list forms are excluded only incidentally. Verified against the live detector across fourteen shapes.
- **A StrictMode crash in the containment guard.** The manifest-derived fallback was an unwrapped pipeline: `Select-Object -Unique` returns a scalar at exactly one distinct home and `$null` at zero, and `.Count` on either throws. The live manifest has ten homes and stayed quiet; every scratch fixture has one, which is the shape that crashed. The derivation is now computed once, array-wrapped, and shared — it had been duplicated byte-for-byte — and the guard conjunct that caused the crash while protecting nothing is gone.
- **`gating_skill` is validated.** It gates which skill's references directory is scanned and nothing checked it, so blanking one field silently removed a containment requirement for a skill reached only that way. The derivation now also reads the existence-checked `exhibits[].file`, and a blank or non-`skills/` value is itself drift.
- **Caller-set entries are normalized.** A path differing only in separator or trailing slash produced a false containment red *and* a false `Composite References` red on a mangled path.
## [3.23.0] — 2026-08-10
...
**`unclassified` is retired rather than discouraged.** `Get-CISuiteSelection` refuses an entry carrying it, by class token — which matters, because 187 of the 188 entries shared one byte-identical sentence and a check keyed on that wording would have passed both polarity arms (the positive and negative test cases) and then waved through every future entry phrased differently. The legal set is now `linux-red`, `never-ci`, and a new `no-signal` for a suite that executes and yields no verdict at all; `no-signal` requires an issue like `linux-red`, because the cheap alternative — widening `never-ci` — would have weakened the one class the whole triage exists to police. The guard this replaces asserted that a count was `-BeGreaterOrEqual 0` and no larger than the set it is a subset of: a tautology and a structural bound, green over a registry of 188 unmeasured entries.
...
**Ten suites were failing on defects that had nothing to do with what they assert**, and the causes were fixed rather than written up as ten misleading quarantine reasons (six `-WindowStyle`, `conductor-body-size`, two more from the `--body-file` mock drift, and `phase-containment-rolling-history-core` — two of the mock-drift four overlap the six):
...
- Seven suites' `gh` mocks still branched on `--body` after #1035 moved `Find-OrUpsertComment`'s new-comment path onto `--body-file`. `--body-file` matches `--body` by prefix, so each mock entered the branch and then `[Array]::IndexOf` found no literal `--body` element and recorded an empty body — or, in one case, the temp-file path. In `frame-credit-ledger-suppress-failed-posts` the test this silently disarmed was the **control** whose only job is to prove the FAILED path is reachable. Four were mis-asserting and are counted among the ten above; adversarial review found three more (`frame-credit-ledger-fail-open`, `frame-credit-ledger-orchestrator`, `cost-integration`) that were latent only because nothing asserted the captured value, and those are closed too rather than left armed inside the expanded gate. The drift was invisible precisely because none of these suites ran in CI — #1035 hardened the one mock that did.
...
**`Get-CISuiteSelection` now refuses a present-but-unusable `issue` on a ticketed class** — a placeholder like `"TODO"`, a zero or a negative — where the check was previously "non-blank" and admitted any of them. A registry that was green can newly redden, which is the point: an entry whose ticket nobody can close is a drop wearing a quarantine's badge. Found by external review (CodeRabbit) after the internal pipeline had completed.
...
The record is composed against GitHub's 65,536-codepoint comment cap with the arithmetic done up front rather than discovered mid-run: classifying detail spills into further comments rather than being truncated to fit, and codepoints are counted as codepoints (an emoji in a failure message cannot fake an overflow). New `Documents/Design/hub-artifact-paths-classification.yml` families for `.github/scripts/Tests/*.json` (which the quarantine registry had always belonged to) and `.github/scripts/audit-controls/*`. **119** Pester tests in `.github/scripts/Tests/ci-glob-audit-core.Tests.ps1`. Nine of them are end-to-end, and eight spawn a real child through the shipped execution path — six of those driving the five shipped controls, two driving purpose-built fixture suites; the ninth re-reads the row the bounded run already produced rather than paying for a second hang. The bound in particular cannot be demonstrated by any in-process assertion.
**What the adversarial review changed before this landed.** A five-pass panel, a defense pass that re-ran the claims rather than conceding them, and a judge ruled **49 findings sustained** against the first draft of this feature; all but one are fixed here and the exception is escalated to the parent. The defects worth naming, because each one made a criterion read satisfied while checking nothing:

- **The standing safety check could never fire.** The reachability arm that asks "is any suite this run recorded as `did-not-complete` reachable by a documented way of running these tests?" resolved the tests root to an absolute path and compared it against row paths the pipeline produces as *relative*, so the prefix test was false for every row, forever. The gate arm keys on names and cannot see the 191 quarantined suites, so for the population the audit exists to attempt **both arms were dead** and the record affirmatively printed `clean: true`. Four passes reproduced it independently. Its unit test passed only because the fixture built both sides from an absolute scratch path the pipeline never produces.
- **One transient `gh` read failure silently reset the observation history.** The reader returned an empty string for a failed read, an unparsable response, and a genuinely absent history alike; the writer then rebuilt from empty and PATCHed the real comment with the reset. Reader outcomes are now four distinguishable states and a read failure **refuses** rather than resets.
- **Captured suite output could forge a record row.** Detail was fenced without escaping, so output containing a fenced block closed the fence early and let arbitrary text render as a genuine row — near-certain by accident in a corpus that asserts on Markdown.
- **The detail kept the wrong end of itself.** Console tail was selected deliberately ("the last thing it printed is where it stopped") and then head-truncated, discarding exactly the stopping point. Truncation is now state-conditional by provenance, ANSI-stripped, and cut on rune boundaries.
- **Five of eleven parity dimensions hard-coded agreement**, including a Pester version outside the gate's own stated window, which R8(a) rules a failure.
- **The size guard counted grapheme clusters, not codepoints**, and passed an 80,000-codepoint body as 40,000.
- **Unaudited suite output reached a public surface unredacted** while the measure job holds a credential for gate parity; captured streams are now scrubbed at capture time, so the artifacts are covered and not only the record.
- **The hardening added *during* this review was itself pinned by nothing.** `Find-OrUpsertComment`'s POST path was moved onto `--body-file` because the audit's record markers embed a run id, so every run necessarily POSTs and there is no PATCH fallback to fall back on. Its test mock matched `--body`, which `--body-file` satisfies *by prefix*, and no assertion ever read the temp file; the PATCH path's `--input` envelope was worse still, its pattern stopping at the comment id and leaving the flag, the path and the JSON unexamined. Measured rather than argued: across eleven mutations of the two transports — empty payload, no payload written at all, truncated payload, clobbered value, bogus flag, and a straight revert to the pre-hardening `--body` form — **ten were green before and all eleven are red now**. Both mock branches now require their real flag and read the file they are handed. `find-or-upsert-comment.Tests.ps1` is itself `unclassified`-quarantined, so this pin does not yet run in CI — which is precisely the condition #1035 exists to measure.
Two findings were **disproved by the defense** and are recorded as such rather than quietly dropped: the partial-artifact upload does carry `if: always()`, and the token-availability dimension answers exactly the question R8(a) allocates to it. One is **escalated to #993** rather than fixed here: the brief's §2 claim 3 justifies the no-drift precondition with a false statement (the population identity is unconditional), but R2's own proof standard mandates that assertion and §6 lists a derivation offered without it as not counting — so the contract is wrong and the executor is right, and only the throw message's false reasoning is corrected here.

**And then a post-fix pass reviewed the fixes, which found sixteen more.** That pass exists because this lane's recorded rate is that a fix is defective roughly one time in three, and it earned its keep: one high, three medium. The two worth naming are both *re-openings of findings the first round had already sustained*, arriving through the doors their own fixes opened. An unobserved environment fact became `$null`, which the parity table then coerced to a polarity — so a row could assert parity on a dimension whose two cells visibly contradicted each other, with the new "checked by" column certifying the fabrication as computed. And the zero-partial record — the path added so a run that measured nothing still produces a record — asserted `clean: true` and a non-contention claim, because both guards are filters over the row set and an empty set satisfies them for free.

The pass also found that the hardened comment-POST path was **completely unpinned**: emptying the body file, or deleting the write entirely, left the suite 17/17 green, because the mock matched `--body` by prefix and `--body-file` satisfied it by luck. Pinning it exposed that the PATCH path was worse — its mock pattern stopped at the comment id, so the payload was never examined at all — and a test whose title promised a stderr assertion its body could not perform, since the library reports through `[Console]::Error.WriteLine`, which bypasses PowerShell's error stream. Ten of eleven mutations were green before; eleven of eleven are red now.
