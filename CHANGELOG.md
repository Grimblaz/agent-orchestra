...
- **The standing check gained a containment clause.** Pinning the receiving-skill set in the caller stopped the manifest shrinking that population, but nothing stopped the caller's set from failing to *cover* the manifest — and an uncovered skill is simply never visited, so its unmanifested-file scan and its `## Composite References` arm were both skipped while the audit reported zero drift. Found by running it: the audit returned no drift both with and without the receiving entry this chunk had to hand-add. Closed with an induced-failure fixture asserting the discriminating pair.

### Fixed

- **`skills/plugin-release-hygiene/SKILL.md` § 5 no longer derives the next version from the branch.** It reads the default branch with a fetch immediately before the read, names the shell the command actually runs in (MSYS argument conversion breaks it under Git Bash), defers to `Get-PRHDefaultBranch` when the ref will not resolve, and refuses the branch-local fallback outright. The re-check now fires before landing rather than only after a merge — `main` advancing underneath an idle branch is not a merge your branch performs, which is how #1050's second failure got through.
- **The same section now states its own bound rather than implying completeness.** Reading the default branch shows what has landed, not what an open PR has claimed, so two branches cut from one base still collide; `release-gate.ps1` is a landing backstop, not a prevention, and is itself suppressed while a PR is conflicted. Structural fix tracked at #864, which the lens now links.
- **The grounding-evidence detector's absolute is removed at all three surfaces that carried it** — the design document, the script docstring, and the tests docstring — rather than only the first. The guarantee is now stated positively (the sentinel must be immediately followed by the bold heading; occurrences inside fenced blocks and inline code spans are excluded, and that pair of exclusions is the whole of it), so the claim can come out false. The script's `HONESTY DISCLOSURE` carries the detection-shape limitation alongside the end-state/backfill one, which is what makes the design document's pointer to it resolve.
