...
- **`/audit-docs` command and mechanical-check script** (`commands/audit-docs.md`, `skills/ai-first-documentation/scripts/audit-docs-mechanical.ps1`, `skills/ai-first-documentation/templates/CLAUDE.md-starter.md`): Consumer-mode enablement for the `ai-first-documentation` skill. New `/audit-docs` command runs deterministic mechanical checks (A2, B2, B3, B5, A9) against any consumer repository with explicit `-Root`, emits JSON results, and supports a waiver convention via `.claude/documentation-decisions.md`. New `init` action bootstraps a minimal CLAUDE.md starter template. New routing row (`intent_key: audit-docs`, audit-anchored patterns) and collision fixtures. SKILL.md updated with `## Consumer-Mode Audits` and `## Recording Documentation Decisions` sections including the H3-per-record decision-record format and CI acquisition guidance (#699).
