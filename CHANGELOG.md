...
- **The first fix for the marker defect did not close it, and a post-fix review pass caught that.** Scoping stanza detection to the header region, then moving the shipped snippet to column 0 so it could be copied, made a verbatim paste of the migration recipe *into a header region* reproduce the original misclassification exactly — and both tests written for it placed their quote below the first heading, so neither touched the one region detection still read. Detection is now fence-aware as well: inside a fenced block is quoting, outside it is declaring. The same pass found four more fixes that closed their exhibit but not the class one step sideways — residue was checked only above the first heading, duplicate-region detection counted only the opening marker, path containment compared case-insensitively (right on Windows, wrong on the Linux runner CI uses), and a trailing separator slipped through to be reported as an unreadable file that "exists". One new guard could not fail at all: its fixture omitted the `limit_observation` row, so the axis short-circuited before reaching the cast that threw. All are fixed, and 11 of the new tests are confirmed red against the tree that preceded them. Regression suite: 32 tests to 99.
