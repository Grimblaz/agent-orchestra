...
The record is composed against GitHub's 65,536-codepoint comment cap with the arithmetic done up front rather than discovered mid-run: classifying detail spills into further comments rather than being truncated to fit, and codepoints are counted as codepoints (an emoji in a failure message cannot fake an overflow). New `Documents/Design/hub-artifact-paths-classification.yml` families for `.github/scripts/Tests/*.json` (which the quarantine registry had always belonged to) and `.github/scripts/audit-controls/*`. **119** Pester tests in `.github/scripts/Tests/ci-glob-audit-core.Tests.ps1`, eight of which drive the shipped controls through the shipped execution path in a real child process — the bound in particular cannot be demonstrated by any in-process assertion.
