...
- The **half-migrated** state — stanza written, policy file not yet — is named as such and reported as a defect, deliberately distinct from a policy file that exists and cannot be read, which refuses. The entry point's parameter surface is unchanged (`-IndexPath`, `-PolicyReferencePath`, `-Json`), so `-Policy` and `-Index` still bind unambiguously, and `-Json` carries the store shape and the whole size axis alongside the text rendering. Regression suite: 32 tests to 69.
