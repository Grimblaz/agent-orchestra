...
The rule now has one home, `review-judgment` § Close-Out Record Amendment, carrying a **lane table** naming the entry document each judge-running lane's reader actually opens — the three `orchestra-review*` commands, `review-github`, Code-Conductor's Review Reconciliation Loop, and Goal-Run's Stage 3 — and, beside it, the paths **excluded with their reasons**: `review-prosecute`/`review-defend` stop before judge, the `design-challenge` and `proxy-github` adapters declare none, and `spine-run` dispatches its `review` port `skill-only`, verifying a row another lane emitted rather than running a judge of its own.
