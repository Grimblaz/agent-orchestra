...
1. Apply `status: parked` or `status: declined` label to the issue via `gh issue edit {NUMBER} --add-label "status: parked"` (or `declined`).
2. Halt further framing. The owner may override by continuing — no separate override mechanism is needed.
