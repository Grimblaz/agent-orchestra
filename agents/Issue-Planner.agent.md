...
After persisting the `<!-- plan-issue-{ID} -->` and `<!-- frame-slices-{ID} -->` comments above, persist the engagement-record marker (see § Named Decisions write-discipline below); immediately after that successful write, persist the credit-input marker — all via the single burst manifest described in § Burst persistence below.
