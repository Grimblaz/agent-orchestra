...
- Claude Code subagent path: `/orchestrate` smart resume may dispatch the `issue-planner` subagent to author or recover a plan. This path keeps the front-load advisory because `AskUserQuestion` calls mid-pipeline may not produce a visible pause. Serialized planner latency for this path is tracked in #457; do not claim latency parity with inline `/plan`.
