...
- **Chunk sub-issues of a designed parent are authored as a brief instead** (`### Brief plan variant` in `skills/plan-authoring/SKILL.md`; doctrine in `Documents/Design/chunked-delivery.md`). A brief carries no numbered steps, Requirement Contracts, execution modes, or per-step validation commands — that is the recipe Bound 2 assigns to the executor's run. Its review charter differs accordingly (#936 D5, sites per DA4): the `#### Brief conformance check` over four mechanical properties, then the prosecution-only `design-challenge` adapter — three lenses, no defense, no judge — not the five-pass panel. Code review's `standard` adapter is untouched by this.
