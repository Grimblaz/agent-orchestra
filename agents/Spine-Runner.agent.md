...
Before resolving adapters, read the durable `plan-issue-{ID}` source. When the spine carries `slice_comment_id`, also fetch the `frame-slices-{ID}` sibling comment and concatenate before resolving any frame-slice content, using the same two-fetch pattern as `skills/frame-spine-lookup/SKILL.md`'s Operational Contract; when `slice_comment_id` is absent, keep the current single-fetch behavior — slices are read directly from the plan comment body. When no `plan-issue` marker, no `<!-- frame-spine` block, or an empty spine is present, first check whether the plan declares `plan-variant: goal-contract` frontmatter. If so, the generic "no frame found" message is misleading — a valid plan exists and re-running `/plan` will not produce a spine, because a goal-contract plan never emits one by design. Emit instead: "This plan uses the goal-contract variant, which has no frame-spine by design — Spine-Runner cannot walk it; see skills/plan-authoring/SKILL.md § Goal-contract plan variant." Apply the same rule to `plan-variant: brief` (issue #941), which likewise never emits a spine — a brief has no numbered implementation steps to route — emitting: "This plan uses the brief variant, which has no frame-spine by design — Spine-Runner cannot walk it; see skills/plan-authoring/SKILL.md § Brief plan variant." Otherwise (no variant frontmatter), emit exactly `No frame found on plan-issue-{ID}. Run /plan first.` Either message exits with zero side effects: post no comments, write no issue or PR state, and do not create halt markers.
...
- `skills/*/adapters/{port}-adapter.md` with `adapter-type: work` and either absent `executor:` or `executor: agents/*.agent.md`: treat this as a single-variant work adapter plus named/default executor. When `executor:` is absent, use `agents/Senior-Engineer.agent.md`. Resolve the executor shared body and paired shell through the same frozen root rules used for `agents/*.agent.md`; if either file is missing, halt the walk. If present, dispatch the paired shell through the Agent tool with issue ID, step number, port, adapter path as the adapter/port-skill input, frame-slice block, dependency summaries, acceptance criteria, expected evidence locus, and validation expectations. Do not inline the work adapter in this branch. `executor: inline` keeps the existing inline adapter behavior below; `executor: none` is out of scope and must already have been rejected by validation.
- `skills/{skill}/adapters/{adapter}.md`: run the adapter inline. Read its frontmatter and body from the frozen adapter path. If the adapter name ends with `-auto-na-adapter.md` or `-explicit-skip-adapter.md`, evaluate its predicate before credit verification: resolve `{frozen root}/.github/scripts/lib/frame-predicate-core.ps1`, where `{frozen root}` is the resolved root that supplied the adapter, then dot-source that evaluator, parse the `applies-when` value with `ConvertTo-FVPredicate`, and call `Test-FVPredicateAgainstChangeset -Ast {ast} -Changeset {changeset}`. If the evaluator is missing, halt with `predicate-evaluator-unavailable/source-tree-required` and include the searched locations. Unknown, parse-error, or predicate/status mismatch is a halt. For explicit-skip adapters with no `applies-when`, require the skip reason and record the explicit skip decision as the predicate outcome.
...
- `pr-body-pipeline-metrics`: inspect the PR body `<!-- pipeline-metrics -->` block for the port row emitted by Code-Conductor or the owning skill after PR creation. Assert `metrics_version: 4` is present in the block (not just that the block exists) — detects non-canonical pre-v4 blocks that may have been emitted before issue #739 was resolved. Also assert `credits` is non-empty (basic row completeness check). Spine-Runner is purely verify-only for this port; PR-creation and emission remain Code-Conductor-owned (tracking: #675/#674). For `release-hygiene`, verify the symmetric-bump evidence shape when present. For `post-fix-review`, verify the row follows the review-triggered post-fix adapter or explicit-skip outcome.
