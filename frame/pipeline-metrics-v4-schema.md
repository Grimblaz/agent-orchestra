...
**Projection provenance.** `cost_summary` projects a subset of fields from the `<!-- cost-pattern-data ... -->` comment block's `schema_version: 1` (defined in `.github/scripts/lib/cost-pattern-data-schema.md`). A reader relying on `cost_summary` should treat it as a v1-schema projection; a future bump to that source schema's `schema_version` has a detectable join point here rather than silently leaving PR bodies carrying stale semantics that still parse without error.
