...
**For standalone work, the expected route is the open-for-work entrance, not this pipeline** (#957, owner decision 2026-07-28): one conversation takes a filed issue to a lawful brief (routine arm) or continues into design (novel arm) — doctrine in [Documents/Design/open-for-work.md](Documents/Design/open-for-work.md). The three-phase pipeline below is reached by **explicitly requesting it** (`/experience` or `/design`) and remains fully lawful. The `/open {issue}` command that invokes the entrance ships with #957 chunk 3; until then, run the open-for-work conversation manually per that document's § Running the flow today.
