...
**For standalone work, the expected route is the open-for-work entrance, not this pipeline** (#957, owner decision 2026-07-28): one conversation takes a filed issue to a lawful brief (routine arm) or continues into design (novel arm) — doctrine in [Documents/Design/open-for-work.md](Documents/Design/open-for-work.md). The three-phase pipeline below is reached by **explicitly requesting it** (`/experience` or `/design`) and remains fully lawful. The `/open {issue}` command that invokes the entrance ships with #957 chunk 3; until then, run the open-for-work conversation manually per that document's § Running the flow today. Three agents cover the pipeline's journey from an issue on the board to an implementation-ready plan. They call each other through durable GitHub-issue markers so a session can span multiple conversations. *(Cross-tool handoff between Copilot and Claude Code was supported; Copilot is now frozen — see Documents/Design/copilot-deprecation.md.)*
