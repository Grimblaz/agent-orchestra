...
- Design context is cached as a GitHub issue comment with `<!-- design-issue-{ID} -->` marker (Claude Code equivalent of the session memory design cache used by VS Code Copilot agents); created by Issue-Planner alongside the plan. If missing at workflow start, read the issue body directly.
