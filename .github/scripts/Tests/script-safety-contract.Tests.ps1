    ...
    It 'script safety: $knownCategories in aggregate-review-scores.ps1 must contain exactly the 7 mandated values' {
        ...
        It 'script safety: test files must not spawn child pwsh processes (use dot-source + in-process call pattern)' {
            ...
            $allowlist = @(
                ...
                'migrate-brief-review-corpus.Tests.ps1',            # IRREDUCIBLE (#968 AC2), and a STRONGER case than the dot-source-guard exemptions above: the five branches under test (line-anchored sibling selection, gh stderr capture, the compare-and-swap before the PATCH, the captured ShouldProcess decision, and the declined-confirmation refusal) all live in migrate-brief-review-corpus.ps1's TOP-LEVEL body, which no in-process call can reach -- same class as phase-containment-emission-check.Tests.ps1's top-level-execution-block exemption. The difference is that this script has NO dot-source guard at all: dot-sourcing it does not merely skip the entry point, it RUNS the whole one-time migration immediately against the live #939 and #941 ledger siblings with ShouldProcess returning true, then calls a top-level exit that kills the Pester host. A declined -Confirm is additionally unreachable in-process, since ShouldProcess needs a real host prompt. Adding a guard and hoisting the body into a function would be a structural change to a script that must be correct on a single irreversible live run
