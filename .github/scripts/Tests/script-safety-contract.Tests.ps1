    ...
    It 'script safety: $knownCategories in aggregate-review-scores.ps1 must contain exactly the 7 mandated values' {
        ...
        It 'script safety: test files must not spawn child pwsh processes (use dot-source + in-process call pattern)' {
            ...
            $allowlist = @(
                ...
                'goal-run-predicate-core.Tests.ps1',                # IRREDUCIBLE (#912 review fix M18): the wrapper CLI smoke test spawns `& pwsh -File goal-run-predicate.ps1` for the same reason as goal-contract-validate-core.Tests.ps1's exemption above -- the wrapper guards its execution block on the same $isDotSourced pattern, so dot-sourcing skips the top-level try/catch that writes the M18 diagnostic to stderr before `exit 2`; only a real subprocess boundary proves the diagnostic actually reaches stderr and the exit code is still 2
                ...
                'migrate-brief-review-corpus.Tests.ps1',            # IRREDUCIBLE (#968 AC2): the branches under test (line-anchored sibling selection, gh stderr capture, the compare-and-swap before the PATCH, the captured ShouldProcess decision, and the declined-confirmation refusal) all live in migrate-brief-review-corpus.ps1's TOP-LEVEL body, which dot-sourcing skips -- same class as phase-containment-emission-check.Tests.ps1's top-level-execution-block exemption above. Two parts are irreducible even so: a DECLINED -Confirm needs a real host prompt, which no in-process call can produce, and the exit-code contract needs a process boundary. Precise about the rest, corrected in review: a call-operator invocation (& the script) WOULD run that top-level body in-process, so "unreachable in-process" is true of dot-sourcing, not of every form. What rules the in-process forms out here is safety, not reachability -- this script has no dot-source guard, so loading it runs the whole one-time migration against the live #939 and #941 ledger siblings with ShouldProcess returning true. Adding a guard and hoisting the body into a function would be a structural change to a script that must be correct on a single irreversible live run
                ...
                'persist-marker-wrapper.Tests.ps1',                 # IRREDUCIBLE (#893 s7): the child-script-shape requirement contract explicitly names `pwsh -File persist-marker.ps1 ...` as one of the two invocation shapes that must be exercised end-to-end (the other is the in-process call operator `&`, which this file also exercises but which does not itself spawn pwsh/powershell and so does not trip this guard) -- dot-sourcing cannot exercise the wrapper's own top-level param()-block CLI entry point, its `exit 0`/`exit 1` contract, or prove the two invocation shapes are actually equivalent; same class as goal-contract-validate-core.Tests.ps1's wrapper CLI smoke-test exemption above
