...
.DESCRIPTION
    ...
    Stop-hook block only counts as a win when the goal evaluator's own
    concurrent decision was 'continue' -- i.e. the loop would have kept going
    absent the hook. When the hook blocks AND the evaluator had independently
    already decided to halt, the hook cannot be credited with beating
    anything; that case is reported as its own distinct outcome
...
function Test-GoalProbeForceHaltWin {
    ...
    .PARAMETER SessionEndDescription
        Hashtable describing how the session ended:
          ...
          StopHookDecision                   [string] 'block' | 'allow' (only meaningful when EndReason -eq 'stop-hook')
          GoalEvaluatorContinuationDecision  [string] 'continue' | 'halt' (only meaningful when EndReason -eq 'stop-hook' and StopHookDecision -eq 'block')
    ...
    .OUTPUTS
        ...
        ('stop-hook-win'|'loss'|'concurrent-halt-not-a-win'|
        'evaluator-decision-indeterminate'|'scope-guard-rejected'),
        Reason [string].
    #>
    [CmdletBinding()]
    ...
    if ($stopHookDecision -ne 'block') {
        return [pscustomobject]@{
            Won     = $false
            Outcome = 'loss'
            Reason  = "Stop hook fired but its decision was '$stopHookDecision', not 'block' -- it did not actually terminate the loop"
        }
    }
    ...
    if ([string]::IsNullOrEmpty($evaluatorDecision)) {
        return [pscustomobject]@{
            ...
            Reason  = "the Stop hook blocked, but the goal evaluator's own concurrent continuation decision was never observed/recorded for this session end -- this is NOT a claim about what the evaluator decided, only that it is unknown"
        }
    }
    ...
    if ($evaluatorDecision -eq 'continue') {
        return [pscustomobject]@{
            ...
            Reason  = 'Stop hook blocked a continuation the goal evaluator itself would have allowed'
        }
    }
    ...
    return [pscustomobject]@{
        ...
        Reason  = "the goal evaluator's own continuation decision was '$evaluatorDecision', not 'continue' -- the hook did not beat anything, it agreed with a decision the evaluator had already made"
    }
}
