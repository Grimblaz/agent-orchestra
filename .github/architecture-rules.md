...
| `.github/scripts/lib/`   | Library modules dot-sourced by CLI wrappers and tests | `{name}-core.ps1` files exposing `Invoke-*` functions; the corresponding CLI script in `.github/scripts/` is a thin wrapper that dot-sources the library and relays results. **Exception**: `skills/session-memory-contract/scripts/persist-phase-ledger.ps1` (issue #878) is a deliberate first-of-kind skill consumer of this directory — see the Allowed dependency list below. |
