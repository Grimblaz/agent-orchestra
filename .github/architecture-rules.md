...
| `.github/scripts/lib/`   | Library modules dot-sourced by CLI wrappers and tests | `{name}-core.ps1` files exposing `Invoke-*` functions; the corresponding CLI script in `.github/scripts/` is a thin wrapper that dot-sources the library and relays results. **Exception**: `skills/session-memory-contract/scripts/persist-phase-ledger.ps1` (issue #878) and `skills/session-memory-contract/scripts/persist-marker.ps1`/`persist-marker-core.ps1` (issue #893) are skill consumers of this directory under the general pattern below — see the [Allowed](#allowed) section under Dependency Rules below. |
| `.github/scripts/audit-controls/` | Deliberately misbehaving Pester suites the full-glob CI audit runs against itself to prove it can tell its four terminal states apart (issue #1035) | `*.Control.Tests.ps1` files plus `README.md`; deliberately **outside** `.github/scripts/Tests/` — `Invoke-Pester .github/scripts/Tests/` is recursive and quarantine-blind, and `never-returns.Control.Tests.ps1` deliberately never returns, so it must never sit beneath the tests root a contributor's pre-PR run walks. Nothing in the repository enumerates this directory for execution; the audit reaches these files only because it is handed their paths explicitly. |
...
- **Pattern — a skill wrapper MAY dot-source `.github/scripts/lib/` directly**, established first by `skills/session-memory-contract/scripts/persist-phase-ledger.ps1` (issue #878) and now also used by `skills/session-memory-contract/scripts/persist-marker.ps1` (issue #893, dot-sourcing `marker-transport-core.ps1`, `frame-engagement-record-core.ps1`, and `frame-spine-core.ps1`) — two independent consumers, no longer a one-off. This is a deliberate skill-to-hub-lib dependency, not an oversight to clean up, and it is safe under a fixed, non-negotiable justification every consumer MUST satisfy: dot-sourcing `.github/scripts/lib` directly from a skill script is safe only because `.github/scripts/lib` ships bundled alongside the consuming script at a fixed relative offset inside the same checkout (hub repo or installed plugin-cache copy) — `$PSScriptRoot` always resolves to the skill's own `scripts/` directory, so a fixed `$PSScriptRoot/../../../.github/scripts/lib` relative path always resolves to the SAME install's own bundled libs at the SAME version as the wrapper, meaning the two can never drift out of lockstep with each other because they are the same install (version-lockstep bundling). This differs from a hypothetical general skill→hub-lib dependency that targets a CONSUMER repo's root (a different, unrelated hazard documented at `skills/session-startup/scripts/session-cleanup-detector.ps1:21-22`) — every skill script dot-sourcing `.github/scripts/lib/` under this pattern must stay inside its own plugin's bundled tree, never resolve into a consumer repo. A future third consumer that satisfies this same fixed-relative-offset justification MAY adopt the pattern without being a fresh exception each time; a consumer that cannot satisfy it (e.g., one that needs a live consumer-repo root) is out of scope for this pattern and needs its own review.
