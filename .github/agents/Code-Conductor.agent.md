...
3. **Execute Each Step**:
   ...
   - **Step commit gate**: If `auto_commit_enabled` is `true`, load `.github/instructions/step-commit.instructions.md` and execute the step commit protocol. If the protocol reports commit failure, annotate the progress checkpoint as `— ✅ DONE (uncommitted)` instead of plain `— ✅ DONE`.
