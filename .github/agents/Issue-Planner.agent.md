...
- Check the issue body for Issue-Designer's CE Gate readiness assessment (customer surface identification, tool availability notes, and pre-drafted CE Gate scenarios — both functional and intent scenarios) — use these directly in the `[CE GATE]` step rather than re-deriving them from scratch.
