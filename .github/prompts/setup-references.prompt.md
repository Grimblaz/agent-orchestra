...
argument-hint: "help, init, generate, validate, undo, dismiss-nudge, optional target repo root"
---
...
If `{{input}}` is empty or the first argument is `help`, show the action list below and stop without writing files.

Resolve the target repository root from the remaining input when a path is supplied; otherwise use the current workspace root. Before running a mutating action, state the target root and the script path that will run.
