...
- `process-retrospective` flagged as TBD — visible in pre-thin/fat #286 but absent in all three other PRs. Deferred-port handling follows the ADR-0004 D14 deferred-skeleton pattern until a later decision lifts it to a first-class port, folds it into `post-pr`, or retires the practice.
