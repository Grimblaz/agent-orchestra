...
- **A brief takes design review's charter.** The full five-pass plan panel does not *relax* for a brief — it stops applying, because the brief is a design-shaped artifact rather than a step-bearing one. What applies instead is the **brief conformance check** over four mechanical text properties (artifact pinning, provenance marking, proof standards present, suite floor checked) plus the prosecution-only `design-challenge` shape — three lenses, no defense, no judge (`skills/plan-authoring/SKILL.md` § Brief plan variant, dispatched from `commands/plan.md` and that skill's stress-test step). Code review's `standard` adapter is untouched by this: the change is adapter *selection* at the dispatch points, never a pass-count edit to a shared adapter. Any *further* movement in review depth — for a brief or for any other plan shape — is still earned by phase-containment-ledger evidence, never by a cost argument (consistent with CLAUDE.md § Quality-first, shift-left); annotate findings in the ledger with whether each was catchable only at plan time. **One honest caveat, recorded rather than glossed** (#947 review; routed to #936 as amendment **DA6**): the ledger's plan-surface writer is judge-gated, and this charter removes the judge stage for a brief, so a brief currently renders as an unverified gap rather than as evidence. Until DA6 defines the brief's emission surface, the instrument this paragraph names is not yet collecting for the lane this paragraph relaxes — which is a reason to hold the depth here, not a licence to relax it further.
...
**If #942 never lands** — #936 makes revocation of the new lane a designed outcome, not a failure — A1–A5 stay, because the amendments bind any chunk plan whatever artifact carries it, and every passage remaining in the table above remains an accurate description of the path still in use. Only this note's *promise* of change would be stale. Retiring it in that case is not #943's stated obligation, which covers the completion path only; revocation routes back to #936, which owns the decision and would say what becomes of this section.
