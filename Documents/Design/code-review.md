...
When `systemic_fix_type` targets agent-orchestra shared files, §4.9 creates an upstream issue after auth check and dedup search (`"[Systemic Fix] {category} [{systemic_fix_type}]" in:title`):
...
- **Target file**: {file in agent-orchestra}
...
- **Part (a) — cold-read**: the shell first cold-reads **the selected review target** directly — the proposed design for a design target, the brief itself for a `plan-variant: brief` target — and records independent observations, before the prompt reveals the 3 finder ledgers. On a brief target it also asks the required vacuity question and carries the answer into the persisted `**Plan Stress-Test**` summary.
...
The § Code Prosecution Workflow → **1. Architecture** checklist (in `skills/adversarial-review/SKILL.md` when this was written; moved to `skills/adversarial-review/modes/code-prosecution.md` by #975's mode split) gains a new checklist item: trace **both** write paths to the persisted target — the branch body plus every downstream helper the branch routes through — and verify by name that the changed branch's path carries every guarantee the sibling's path has (write-time preflight, schema validation, existence checks, post-write verification). A guarantee living only in a sibling's downstream call chain still counts as that sibling's guarantee. This wording is deliberately full-write-path, not branch-local, because the #878 F1 exemplar's sibling guarantee lived in a downstream call, not the branch body — a branch-local comparison would have missed it.

The **2. Security** perspective's "Full-record overwrite risks" bullet (line 120 of `skills/adversarial-review/SKILL.md` when this was written; moved to `skills/adversarial-review/modes/code-prosecution.md` by #975's mode split) cross-references the Architecture item by section anchor (`§ Code Prosecution Workflow → 1. Architecture`) rather than restating the check, so the two perspectives cannot drift apart.
