...
|---|----------|--------|-----------|
| D1 | CE Gate executor | Code-Conductor delegates to Experience-Owner, who exercises scenarios and captures evidence; Code-Critic (CE prosecution) evaluates adversarially | Separation of execution (Experience-Owner, delegated by Code-Conductor) and review (Code-Critic) avoids the fox-guarding-henhouse problem. CE Gate = "delegate → exercise → prosecute." |
...
| D5 | CE Gate vs E2E distinction | CE Gate = workflow phase (agent exercises scenarios). E2E = test type (code artifacts). Distinct concepts. | Solution-Designer's testing scope table unchanged; the E2E column indicates whether test code should be written; CE Gate is when Experience-Owner exercises the scenarios (delegated by Code-Conductor) |
...
A phase that runs after the Validation Ladder and Code-Critic review, before PR creation. Code-Conductor delegates to Experience-Owner, who exercises CE scenarios — the plan describes them in natural language, and Experience-Owner uses the right tool for the surface:
...
| `.github/agents/Solution-Designer.agent.md` | Customer surface identification; CE Gate scenarios and design intent extraction moved to Experience-Owner.agent.md |
...
Experience-Owner (delegated by Code-Conductor) exercises scenarios and captures evidence on two dimensions; Code-Critic (CE prosecution mode) evaluates that evidence adversarially; Code-Review-Response judges:
...
1. **Code-Conductor delegates to Experience-Owner** — Experience-Owner navigates the surface, captures evidence (screenshots, response bodies, CLI output)
...
```text
Code-Conductor delegates to Experience-Owner (CE Gate evidence capture)
  → Experience-Owner exercises scenarios + captures evidence
