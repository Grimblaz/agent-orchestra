...
| D5 | Issue closure | `Closes #N` in PR body | GitHub auto-close is sufficient for an ordinary issue — no summary comment needed. **Scoped by #1013**: an issue carrying an open-for-work affirmation record still owes a close-out record, written before the PR-creation action, or before the close whenever that first moment did not already produce it — `skills/plan-authoring/SKILL.md` § The close-out obligation on an affirmation-record issue, shape at `skills/post-pr-review/SKILL.md` § 9. The keyword itself stays, and note it is *this* row's mechanism that reaches the second moment: a keyword closing a parent from a chunk's PR closes an issue no run of its own was working | Hook |
...
6. The detector evaluates cleanup signals in this order:
   ...
   - **Orphan branches**: detects unattached merged `claude/*` no-upstream branches and unattached upstream-deleted `feature/issue-*` branches (squash-merged branches recognized via `git diff` tree-equivalence since issue #513, or via a merge-tree no-op since issue #922); cleanup commands are inside the fenced block. Zero-unique-commit orphan branches route through the evidence-gated `Test-WorktreeBranchRemovalEligible` primitive (issue #889 — see § Worktree & Branch Removal Eligibility below) rather than trusting trivially-true tree-equivalence.
   - **Fail-open behavior**: fetch, worktree-list, for-each-ref, and ref-lookup failures skip unverifiable candidates without failing the startup flow.
