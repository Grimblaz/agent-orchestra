...
| D9 | Instruction/skill replacement | First `session-startup.instructions.md`, now the `session-startup` skill with the same trigger stub in `.github/copilot-instructions.md` | Delivery moved from an always-loaded shared instruction to a plugin-distributable skill while preserving the same agent-mediated confirmation model (D2), no VS Code version gate, and the same `WORKFLOW_TEMPLATE_ROOT`-based silent-skip behavior before the script runs | Instruction → Skill |
...
| `.github/skills/session-startup/SKILL.md` | Current detailed protocol for the startup self-check; preserves the run-once marker, fail-open behavior, and detector/script path rules while `.github/copilot-instructions.md` remains the trigger stub | Active |
