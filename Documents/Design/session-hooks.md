...
| D22 | Per-line wording surface | Detector adds `; will auto-resolve at cleanup time` suffix for `feature/issue-N-*` orphans; cleanup script splits skip wording into `auto-resolve declined`, `could not verify GitHub signals`, and `became unmerged between re-check and force-delete` | Preamble-only annotation easy to skim past; universal suffix misrepresents `claude/*` orphans; both-surfaces provides per-line visibility at detector time and precise skip context at cleanup time | Auto-resolve |
