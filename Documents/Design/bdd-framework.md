...
| Condition | Behavior |
|-----------|----------|
| Line-start heading present + `bdd: {framework}` recognized | Phase 2 active |
| Line-start heading present, no `bdd:` line | Phase 1 (existing behavior, unchanged) |
| Line-start heading present + `bdd: true` | Warning emitted; Phase 1 fallback |
| Line-start heading present + `bdd: {unknown}` | Warning emitted; Phase 1 fallback |
...
- Consumer opt-in via `## BDD Framework` line-start heading (column 0); template ships BDD-disabled
