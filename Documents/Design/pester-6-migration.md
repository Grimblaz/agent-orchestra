...
| File | Role |
| --- | --- |
| `.github/workflows/pester.yml` | Install/Import lines carry the `-MinimumVersion 6.0.0 -MaximumVersion 6.999.999` window (both lines); guard selected by the CI glob (it carries no `ci-quarantine.json` entry) |
...
`.github/scripts/Tests/pester-mock-fallthrough-guard.Tests.ps1` is a new meta-test, modeled on `script-safety-contract.Tests.ps1`'s precedent (AST-based scan — not text/regex over source — a centralized `$allowlist` with per-entry justification, self-exclusion, and falsifiability fixtures), selected by `pester.yml`'s glob, carrying no `ci-quarantine.json` entry. It enforces three clauses:

1. **Fall-through shape.** Any command with ≥1 filtered `Mock -ParameterFilter` and no same-file default (unfiltered) `Mock` is flagged, unless the `(File, Command)` site is allowlisted. This fires regardless of whether a `global:<cmd>` catch-all is present, because most of the repository's test files did not run in CI when this was designed — static coverage is the only signal available for a file CI does not execute. **The mechanism that keeps files out of CI has since changed, and this sentence has been corrected to say so** (issue #1050): there is no *run list*. Since PR #988, `pester.yml` selects by **glob minus `.github/scripts/Tests/ci-quarantine.json`**, so every suite on disk runs unless it carries a classified quarantine entry. The excluded population was still large when that correction was written — measured at `ec7bc60`, **191 quarantine rows against 255 suites on disk**. **It is no longer** (issue #1036): the quarantine is **14 rows against 255**, so 241 suites now execute in CI and the "static coverage is the only signal available" premise is false for almost all of them. The mechanism is kept anyway, on a narrower and now-explicit basis: static analysis still reaches the 14 excluded suites and, more importantly, fires at authoring time on a file whose defect a green run would not reveal. Do not cite the size of the excluded population as its justification — that argument has now been falsified twice, once by PR #988's selection change and once by #1036's promotion, which is itself the reason to state the mechanism's own merit instead.
...
- **Docs-only convention or runtime-throw-only guard** — an unenforced convention erodes over time, and a runtime throw never fires in CI for a file CI does not execute; a default mock would silence the throw entirely for those files (the exact gap the escalated finding F3 identified). *(Corrected under issue #1050: this read "the ~166 files outside the CI-scoped run list". The run list was retired by PR #988 in favour of a glob minus `ci-quarantine.json`. **Corrected again under issue #1036**, which took the quarantine from 191 rows to 14: "a file CI does not execute" now describes 14 suites, not 191, so this alternative is rejected on the strength of the enforced guard rather than on the size of the unexecuted population.)*
