...
1. **Fall-through shape.** Any command with ≥1 filtered `Mock -ParameterFilter` and no same-file default (unfiltered) `Mock` is flagged, unless the `(File, Command)` site is allowlisted. This fires regardless of whether a `global:<cmd>` catch-all is present, because most of the repository's test files did not run in CI when this was designed — static coverage is the only signal available for a file CI does not execute. **The mechanism that keeps files out of CI has since changed, and this sentence has been corrected to say so** (issue #1050): there is no *run list*. Since PR #988, `pester.yml` selects by **glob minus `.github/scripts/Tests/ci-quarantine.json`**, so every suite on disk runs unless it carries a classified quarantine entry. The excluded population is still large — measured at `ec7bc60`, **191 quarantine rows against 255 suites on disk** — so the reasoning above survives; what changed is that the exclusion is now an enumerated, reason-carrying backlog rather than an unlisted default.
...
- **Docs-only convention or runtime-throw-only guard** — an unenforced convention erodes over time, and a runtime throw never fires in CI for a file CI does not execute; a default mock would silence the throw entirely for those files (the exact gap the escalated finding F3 identified). *(Corrected under issue #1050: this read "the ~166 files outside the CI-scoped run list". The run list was retired by PR #988 in favour of a glob minus `ci-quarantine.json`; the argument is unchanged, since 191 of 255 suites are quarantined at `ec7bc60`.)*
