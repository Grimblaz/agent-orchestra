...
Work moves through up to eight beats. For standalone work, the expected entrance is **opening the issue for work** (#957; [Documents/Design/open-for-work.md](Documents/Design/open-for-work.md)) — one conversation that grounds the problem, gets an affirmed statement of what is being built, and then routes to a brief (routine work) or continues into design (novel work). The command that invokes that entrance is `/open {issue}`; the methodology it runs lives in [skills/open-for-work/SKILL.md](skills/open-for-work/SKILL.md). Steps 2 and 3 below (customer framing and technical design) are reached by explicitly requesting them (`/experience` / `/design`) — or, for design, by the open-for-work conversation's novel verdict continuing into it inline — and remain fully lawful. Step 6 (adversarial review) has two variants: full (five-pass panel, then defense, then judge) and lite (one compact prosecution pass, then defense, then judge).
...
| --- | --- | --- |
| Interaction model | Interactive — engagement gates (scope classification, plan approval, standards checks) fire live and wait for an answer | Unattended — the run never asks the operator anything mid-run, in any form; any point that would otherwise need a live answer produces a typed halt instead |
...
| `gate-input-needed` | The run hit a decision that genuinely needs a human answer — the machine-safe substitute for the escalation the conducted pipeline would raise, since goal-run cannot ask mid-run | A chain-gate demand (the trigger itself is separate follow-on work, #848 E1) at a point the harness cannot safely resolve alone | The human operator — read the halt report's `plan_remediation` field, answer directly, then re-invoke `/goal-run {issue}` to resume |
...
| **chunked delivery** | Splitting a designed parent issue into plan-only sub-issues ("chunks"), each one sub-issue → one plan → one pull request. The parent designs the seams between chunks; the chunks decide their own internals. | `Documents/Design/chunked-delivery.md` |
| **Bound 1 / Bound 2** | The two detail limits chunked delivery enforces. Bound 1: the parent design stops at the seams and never designs a chunk's internals. Bound 2: the chunk plan is a contract, not a recipe. | `Documents/Design/chunked-delivery.md` |
| **brief (chunk plan)** | The plan shape declared by `plan-variant: brief`: six numbered sections stating the contract — problem, epistemic map, acceptance criteria, falsifiers, context inventory, evidence obligations — and no implementation steps. Authored under either of its two authority sources: a chunk sub-issue of a designed parent, or a standalone issue carrying an affirmed open-for-work framing record **together with a routine beat-2 verdict** (#957 D4/Amendment 10 — a novel verdict continues into design instead). Distinct from the "context brief" upstream-onboarding renders. | `skills/plan-authoring/SKILL.md § Brief plan variant` |
| **A1–A5 (doctrine amendments)** | Five rules governing how a brief is allowed to specify, whichever of its two authority sources it carries (#957). A1 discovery-neutral targets; A2 provenance-marked grounding (source-read vs sample-inferred); A3 falsifiers as executor prose, never a graded check; A4 behavior pins plus a pre-launch floor check; A5 evidence obligations. | `Documents/Design/chunked-delivery.md § How a chunk plan specifies` |
| **DA{N} (parent-design amendments)** | An amendment to a parent issue's design, recorded after that design phase closed. Numbered `DA{N}` deliberately so it does not collide with the doctrine amendments `A1`–`A5`. | The `## Design Amendments` section of the parent issue |
| **discriminating / attributed / per-criterion** | The three properties any completion evidence must carry (A5). Discriminating: it could have come out negative. Attributed: it says where the number came from. Per-criterion: it maps to one acceptance criterion, not to one aggregate green. | `skills/verification-before-completion/SKILL.md` |
...
| **load-bearing (decision)** | A classification for a named decision that materially changes the customer experience or architecture — requires a full decision brief with options, recommendation, and audit rationale, put explicitly to the maintainer. | `skills/solution-authoring/SKILL.md` |
...
| **D1 / D2 / D3 (auto-mode rules)** | Named auto-mode boundary rules: D1 = routine ops auto-approve, D2 = outside-allowlist ops prompt, D3 = an engagement gate fires unconditionally. | `CLAUDE.md § Auto-mode boundary` |
