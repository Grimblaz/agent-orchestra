...
Work moves through up to eight beats. For standalone work, the expected entrance is **opening the issue for work** (#957; [Documents/Design/open-for-work.md](Documents/Design/open-for-work.md)) — one conversation that grounds the problem, gets an affirmed statement of what is being built, and then routes to a brief (routine work) or continues into design (novel work). The `/open` command that invokes that entrance ships with #957 chunk 3; until then the conversation runs manually per that document's § Running the flow today. Steps 2 and 3 below (customer framing and technical design) are reached by explicitly requesting them (`/experience` / `/design`) — or, for design, by the open-for-work conversation's novel verdict continuing into it inline — and remain fully lawful. Step 6 (adversarial review) has two variants: full (five-pass panel, then defense, then judge) and lite (one compact prosecution pass, then defense, then judge).
