...
| **brief (chunk plan)** | The plan shape a chunk sub-issue is authored in (`plan-variant: brief`): six numbered sections stating the contract — problem, epistemic map, acceptance criteria, falsifiers, context inventory, evidence obligations — and no implementation steps. Distinct from the "context brief" upstream-onboarding renders. | `skills/plan-authoring/SKILL.md § Brief plan variant` |
| **A1–A5 (doctrine amendments)** | Five rules governing how a chunk plan is allowed to specify. A1 discovery-neutral targets; A2 provenance-marked grounding (source-read vs sample-inferred); A3 falsifiers as executor prose, never a graded check; A4 behavior pins plus a pre-launch floor check; A5 evidence obligations. | `Documents/Design/chunked-delivery.md § How a chunk plan specifies` |
